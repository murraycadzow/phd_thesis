...
### Objectives
 ...
 - confirm the genetic relationship between 
...
Of the genes that were in the windows of the 1st percentile, there were 29 genes that were in the list of genes of metabolic disease associted genes and also had windows that intersected them from the Polynesian populations. Thirteen of those genes were for windows from Polynesian populations, as well as others. Some examples of these genes were RASA2, which had windows only from \gls{cim} nd \gls{nzm}. ERBB4, which had windows from all four Polynesian populations, as well as the populations of \gls{eas} - except \gls{jpt}.
...
rand_tab_pander %>% kable(., caption = '(\\#tab:randtab) Proportion of individual populations assigned to their super population by hierarchical clustering across 10000 iterations of 2500 randomly drawn windows.', booktabs = TRUE, col.names =  c('Super Population', 'Min','Mean','Std. Dev','Max','Prop50 (%)', 'Total Complete (n)') ) %>% footnote(general = "Prop50 is the percentage of the iterations that had at least 50\\\\% of each super population assigned the same cluster. Total complete is the number of iterations all populations of a super population were assigned the same cluster", threeparttable = TRUE, general_title = "", escape = FALSE)
...
Objective 1:
can the seelction and neutrality statistics be used to group the 

- reiterate the migration

The history of the Polynesian migrations is the out of Africa, up through europe, and out from East Asia into Polynesia. Polynesia is settled in two waves, Western and then Eastern.

The resutls that we get from the FST on the entire chromosomes gives the same pattern, first it matches the 1000 genomes project and, secondly, specifically the Polynesian populations see a grouping in the East and West. This was also seen in XXX.

The grouping is not seen with the whole chromsome results. 


In Chapter 3 it was noticed that the populations had population specific distributions for the frequency spectrum based selection and neutrality statistics. The tails of the distributions are the most extreme and there most likely to be the loci that have undergone selection - specifically the negative tail is of interest for positive selection. It was thought that perhaps the tails could be used to group the individual populations using an unsupervised method - in this case, hierarchical clustering - and reassemble the super populations. The first attempts used the entire chromosome and created a summary statistic that was used to cluster. It was successful in reassmebling the super populations using F~ST~ which is a measure of population differentiation, and had previously been used with in the \gls{1kgp} dataset to establish how differentiated the \gls{1kgp} samples were from each other. The population grouping were also the same as in the \gls{pca}  that was used to select the individuals for the selection analysis (Figure \@ref(fig:allpopPCA)). The whole chromosome summary using the intra-population frequency spectrum neutrality and selection statistics had varying levels of success in re-assembling the super populations. Some, such as \gls{fld} and \gls{flf} had mini-groups of 2-3 populations that were part of the same super population, but these mini-groups were not closely linked to the other populations of the same super population. These two statistics are based on singletons, and it is likely that these are not well represented in the markers of the CoreExome, or the quality control that the New Zealand samples underwent, leading to an under-representation of the singletons that actually might be present in the populations.


Overall, it was found that the clustering of selection and neutrality test statistics for entire chromosomes did not resemble the grouping that clustering on F~ST~ produced. However, using windows from the extremes of the distribution was effective at clustering the individual populations into their super population groups. \gls{fwh} was the only statistic that grouped all individual populations into their corresponding super populations, whereas \gls{td} and \gls{ze} had the same split of the \gls{amr} populations as with the F~ST~ clustering. The haplotypic based methods of \gls{ihs} and \gls{nsl} clustered into super populations and had the \gls{amr} population split too. There was not a high amount of region sharing outside of each super population. Clustering from gene lists consistently grouped the individual populations into their super populations although there was inconsistency for the \gls{amr} populations. With the exception of the \gls{amr} populations, the clustering of populations using windows or haplotypes was consistent with previously reported results [@1KGP2015snp; @pickrell2009signals]. 



With the extremes, for many of the super populations had many windows that were not in the extremes of the other populations, these formed the 'blocks' in the heatmap. This was most evident with the \gls{afr} populations, which had a good deal of the windows for each statistic not in common with the extremes from the other populations.


Prior to this study there has only been a few studies that have genetically established the relationship of the Polynesian genome with respect to other populations. Recent studies have done this predominantly though the mitochondrial and Y chromosome lineages. The admixture analysis they performed 

The anaylsis is important because it adds evidence to the genetic relationship between the Polynesians and the other populations. 

Other studies have previously shown the ancestral lineages of the Polynesians with relation to the other populations, however this was done predominantly with the mitochondrial and Y chromosome lineages [\@ref]. The relationship using the autosome has aslo been done but using very small sample sizes. This is study is the first to compare four Polynesian populations, two of East, and two of West Polynesian ancestry, and of similar sample sizes to those of the \gls{1kgp}.

Understanding the ancestry of populations helps to inform on
...
- What we see is there is a higher burden of metabolic disease in Polynesians and not in other populations. Could it be that the selective pressure occured after the migratory events?
...
ie do we see similar frequencies of disease alleles
...
- because we know the migration history, and we also have shown that the modern day populations still show these groupings that correspond to temporal and spacial geography it can help us infer when a particular region of the genome may have had a selective event.

- why does it matter when something happened though?
    - distigmatisation
    
- why? by comparing populations it might give an idea of particular regions that might explain differences in prevalence and or severity/likelyhood of developing metabolic disease
...
The clustering based on the median centred values of the statistics was able to reassemble the super population goups. However, the groups were also reassembled with the random draws of genes, with an increase in the number of completely grouped super populations when more genes were used.

limitations

- the fact it was a chip
...
- the genes that do share, what are they associated with? and what do th prevalences of those populations look like?
- 

...
```{r tdTop1000, fig.cap = '(ref:tdTop1000)',echo = FALSE}
...
```{r fwhTop1000, fig.cap='(ref:fwhTop1000)'}
...
```{r fldTop1000, fig.cap = '(ref:fldTop1000)'}
...
```{r flfTop1000, fig.cap = '(ref:flfTop1000)'}
...
```{r zeTop1000, fig.cap = '(ref:zeTop1000)'}
...
Clustering on Zeng's E on the positive tail of the distribution gave clustering that matched the super popualations, with the Polyesian populations grouped together and subgrouped into an East/West split. The Polyensian population group's next closest group was the \gls{eas} super population. The negative tail showed a similar grouping but split the \gls{amr} super population.
...
#### Random draws
```{r}
td_random <- readRDS('~/data/NZ_coreExome_1kgp/30kbWindow_intra/td_random-28-6-2017.RDS')
bind_rows(td_random) %>% group_by(super) %>% summarise(min = min(max_prop), mean = mean(max_prop), sd = sd(max_prop), max = max(max_prop), complete_prop = sum(if_else(max_prop == 1, 1, 0))/NROW(.))
```
...
(ref:ihsclust) _\gls{ihs} Clustering_ Clustering based on proportion of \gls{ihs} regions shared between populations
...
(ref:nslclust) _\gls{nsl} Clustering_ Clustering of \gls{nsl} based on proportion of regions shared between populations.

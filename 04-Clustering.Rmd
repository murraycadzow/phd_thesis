...
Hierarchical clustering for each selection test statistic using the most extreme 1% windows by value for the frequency spectrum based methods and proportion of region overlap for the haplotypic methods grouped inidiviual populations into each of their super population groups was only done by \gls{fwh}. If an allowance is made for the 2 \gls{amr} populations to be grouped with the \gls{eur} populations, as the chromosomal F~ST~ clustering showed, then both haplotypic methods and 5 of the frequency spectrum based methods tails clustered super populations into their respective super populations.  
...
ihs:
the populations with the least regions shared was \gls{asw} and \gls{cim} with `r sprintf("%.1f",mat_ihs['POL_CIM','AFR_ASW'] *100)`%  shared.
the populations that had the most amount of region shared was \gls{nzc} and \gls{ceu} with `r sprintf("%.1f",mat_ihs['EUR_NZC','EUR_CEU'] *100)`% shared.

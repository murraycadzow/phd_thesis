...
Not a single random draw grouped the all individual populations into their super population. Only for the Polynesians populations and \gls{amr} fully clustered into the same cluster and this was a rare event. Grouping at least 50% of a super population was at best `r round(rand_tab %>% summarise(max(prop50)), digits = 0)`% of the draws but for the larger super populations (\gls{afr}, \gls{eas}, \gls{eur}, and \gls{sas}) it was on average `r round(rand_tab %>% filter(!super %in% c('POL','AMR'))%>% summarise(mean(prop50)), digits = 0)`% of the draws.

There was only `r bind_rows(lapply(td_random, function(x){ x[['super_prop']] %>% spread('super', 'max_prop')})) %>% filter(POL >= 2/4, EUR >= 3/6, EAS >= 3/5, AFR >= 3/6, SAS >= 3/5) %>% arrange(POL) %>% NROW()` times that all super populations each had half or more of their individual populations assigned the same group.
...
```{r, eval = FALSE}
#exclusivity
bind_rows(lapply(seq_along(td_random)[1:1000], function(x){td_random[[x]]$exclusivity %>% t() %>% data.frame %>% mutate_all(., funs(replace(., . == 0, NA))) %>% summarise_all(., funs(mean(., na.rm=TRUE)))})) %>% summarise_all(mean)
...
#prop
bind_rows(lapply(names(prop_list), function(x){prop_list[[x]]$super_prop %>%  spread(super, max_prop) %>% mutate(stat = x) %>% select(NCOL(.), 1:(NCOL(.)-1)) }))

#exclusivity

bind_rows(lapply(names(prop_list), function(x){prop_list[[x]]$exclusivity %>% data.frame%>% mutate(pop = row.names(.)) %>% gather("cluster","prop",1:6) %>% filter(prop > 0) %>% group_by(pop) %>% summarise(mean_prop = mean(prop)) %>% spread(pop, mean_prop) %>% mutate(stat = x) %>% select(NCOL(.), 1:(NCOL(.)-1)) })) %>% mutate_if(., is.numeric, funs(round(., 3)))
```

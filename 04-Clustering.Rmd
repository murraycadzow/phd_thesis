...
```{r, fig.cap = "(ref:tdDensity)", echo = FALSE}
knitr::include_graphics("images/04_clustering/td_densities.png")
```
(ref:tdDensity) Plot of the distribution of \gls{td} by population. Triangles indicate the minimum and maximum values. Dots indicate the lower and upper thresholds for each population.
...
pol_tail_filter('td_pos', pol_min = 2, all_max = 4) %>%  pull(SYMBOL) %>% unique() %>% sort()
```
...
In both tails the clustering is being driven by a small subset of the regions, this can be seen in Figures \@ref(fig:tdTop1000) C and D where most of the regions that the Polynesian populations are positive for, are not shared with the other populations. It can also be seen that the regions that the Polynesian populations do have in common are with the \gls{eas} populations. This is reflected in the dendrograms in Figures \@ref(fig:tdTop1000) A and B.
...
```{r fwhDensity, echo = FALSE}
knitr::include_graphics("images/04_clustering/fwh_densities.png")
```

(ref:fwhDensity) Plot of the distribution of \gls{fwh} by population. Triangles indicate the minimum and maximum values. Dots indicate the lower and upper thresholds for each population.
...
(ref:fldDensity) Plot of the distribution of \gls{fld} by population. Triangles indicate the minimum and maximum values. Dots indicate the lower and upper thresholds for each population.

```{r, fig.cap = "(ref:fldDensity)", echo = FALSE}
...
(ref:flfDensity) Plot of the distribution of \gls{flf} by population. Triangles indicate the minimum and maximum values. Dots indicate the lower and upper thresholds for each population.

```{r, fig.cap = "(ref:flfDensity)", echo = FALSE}
...
(ref:zeDensity) Plot of the distribution of \gls{ze} by population. Triangles indicate the minimum and maximum values. Dots indicate the lower and upper thresholds for each population.

```{r, fig.cap = "(ref:zeDensity)", echo = FALSE}
...
rand_tab_pander %>% kable(., caption = '(\\#tab:randtab) Summary of individual populations assigned to their super population using 10000 iterations of 2500 randomly drawn windows.', booktabs = TRUE, col.names =  c('Super Population', 'Min','Mean','Std. Dev','Max','Prop50 (%)', 'Total Complete') ) %>% footnote(general = "Prop50 is the proportion of the iterations that had at least 50\\\\% of the populations clustered into their respective super population. Total complete is the number of iterations all populations of a super population were assigned the same cluster", threeparttable = TRUE, general_title = "", escape = FALSE)
...
In order to test if the clustering from gene lists was specific to the disease associated genes used, lists of random genes were created and then clustered. Random gene lists were generated consisting of either 25 or 100 genes, 100 times each, and median centred based on the population for \gls{td}, \gls{fwh}, \gls{fld}, \gls{flf}, and \gls{ze}. Hierarchical clustering was then performed on the windows that intersected genes from the gene lists. The number of times the Polynesian super population was completely clustered is given in Table \@ref(tab:randgenes). The  Polynesian super population was completely clustered more often with the lists of 100 genes for \gls{td}, \gls{fwh}, \gls{flf}, and \gls{ze} compared to the lists of 25 genes. The number of times all populations were clustered completely was less than 10%. However, if the \gls{amr} super population is not included then it becomes 44.8 for the 100 genes, and 29 for the 25 genes. \Gls{fld} and \gls{flf} did not perform well as clustering the Polynesian populations into their super population, and only achieved this less than 5% of the time. Clustering all populations was not achieved. The other frequency spectrum statistics clustered the Polynesian populations into their super population at least 50% of the time and did better with more genes. Clustering of the other super populations was similar to the Polynesian super population when the \gls{amr} populations were not included. This indicates that clustering from disease associated gene lists is not specific to particular genes. It also indicates that genes are better at clustering the super populations than randomly drawn windows.

...
urate_ihs_snps <- subset_snps(sig_ihs_val_GR, gr, urate_goutGR)
obesity_ihs_snps <- subset_snps(sig_ihs_val_GR, gr, obesity_GR)
t2d_ihs_snps <- subset_snps(sig_ihs_val_GR, gr, t2d_GR)
kd_ihs_snps <- subset_snps(sig_ihs_val_GR, gr, kd_GR)
metsyn_ihs_snps <- subset_snps(sig_ihs_val_GR, gr, metsyn_GR)


urate_nsl_snps <- subset_snps(sig_nsl_val_GR, gr2, urate_goutGR)
obesity_nsl_snps <- subset_snps(sig_nsl_val_GR, gr2, obesity_GR)
t2d_nsl_snps <- subset_snps(sig_nsl_val_GR, gr2, t2d_GR)
kd_nsl_snps <- subset_snps(sig_nsl_val_GR, gr2, kd_GR)
metsyn_nsl_snps <- subset_snps(sig_nsl_val_GR, gr2, metsyn_GR)
...
# n snps
n_pol_snps_ihs <- bind_rows(urate_ihs_snps, obesity_ihs_snps, t2d_ihs_snps, kd_ihs_snps, metsyn_ihs_snps) %>% filter(superpop == 'POL') %>% select(seqnames, start, end) %>% distinct() %>% NROW()

n_pol_snps_nsl <- bind_rows(urate_nsl_snps, obesity_nsl_snps, t2d_nsl_snps, kd_nsl_snps, metsyn_nsl_snps) %>% filter(superpop == 'POL') %>% select(seqnames, start, end) %>% distinct() %>% NROW()

n_pol_snps_ihs_nsl <- inner_join( bind_rows(urate_ihs_snps, obesity_ihs_snps, t2d_ihs_snps, kd_ihs_snps, metsyn_ihs_snps) %>% filter(superpop == 'POL') %>% select(seqnames, start, end) %>% distinct(),
              bind_rows(urate_nsl_snps, obesity_nsl_snps, t2d_nsl_snps, kd_nsl_snps, metsyn_nsl_snps) %>% filter(superpop == 'POL') %>% select(seqnames, start, end) %>% distinct()
             ) %>% NROW() 
...
Using the significant \gls{snp}s for these \gls{ihs} and \gls{nsl} (as in Chapter 3) there were a total of XXX \gls{snp}s that were significant in a Polynesian population and also significant for the same allele in at least one other population for \gls{ihs}, and XXX for \gls{nsl}
...
The \gls{sam} and \gls{ton} populations had multiple \gls{snp}s in common that were significant for both \gls{ihs} and \gls{nsl}. 

Of the metabolic disease genes, ERBB4, FHIT, and ITGB6 were also found in @pickrell2009signals.

pickrell ERBB4 in eas, sas, and mideas, oceania. FHIT in bantu. ITGB6 in america
...
> **check the numbers for gene clustering once fld is removed**
...
### Objective 1: can the selection and neutrality statistics be used to group populations?
...
Allele sharing between populations is moderate for common \gls{snp}s, and more so for contiental groups, but is minimal for rarer frequencies, due to population divergence [@Gravel2011]. This helps to explain the patterns of window sharing between the \gls{sfs} based statistics.


> need some more info about ihs/nsl
...
This means that while there were similarities in the extremes of the distributions for the frequency based statistics, they are not necessessarily under selection.

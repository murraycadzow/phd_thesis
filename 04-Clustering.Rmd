...
Admixture analysis is another method to infer the number of ancestral populations that contribute to current populations by modelling the probability of the observed genotypes using proportions of ancestral populations and the population allele frequencies [@Alexsander2009] 



There were two analyses for admixture that were undertaken. The first was using all of the populations to calculate the number of ancestral populations. Five-fold cross-validation was used to identify the K value with the lower cross-validation error. K = 11 had the lowest cross-validation error (Table \@ref(tab:admixCV)).
...
(ref:admix) Proportions of ancestral populations as inferred from ADMIXTURE using K = 11.

```{r, echo = FALSE, fig.height = 9, fig.width = 6, fig.cap = '(ref:admix)'}
# barplot of the reference proportions
cbind(ref_fam, ref) %>% gather(., "V","Q",9:NCOL(.)) %>% mutate(V = forcats::as_factor(V)) %>% mutate(super_sample = paste0(super_pop,'_',sample), facet_pop = paste(super_pop, pop)) %>% ggplot(., aes(x = super_sample, y = Q)) + geom_bar(stat = "identity", position = 'stack', aes(fill = V), width = 1) + facet_wrap(super_pop~pop, scales = "free_x", ncol = 6, shrink = TRUE) + theme_bw() + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank()) + scale_y_continuous(expand = c(0,0)) 
rm(ref,ref_fam)
```



The second analysis used source populations of \gls{afr}, gls{eur}, \gls{eas}, \gls{sas} to calculate the 

 K = 9 provided the smallest error and so was used as the input reference ancestry proportions to be projected onto the populations of \gls{amr} and \gls{pol}.
...
knitr::kable(k_cv, caption = "(\\#tab:admixCV) Admixture cross-validation (CV) error for different values of K. Projection CV error is the admixture cross-validation error for different K values from projecting the components from 1000 Genomes Project populations of \\gls{afr}, \\gls{eas}, \\gls{eur}, and \\gls{sas} onto the populations of \\gls{amr} and \\gls{pol}.")
```

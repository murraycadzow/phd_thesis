...
gr <- subsetByOverlaps(gr_ihs, urate_goutGR)
left_join(gr %>% as.tibble(),txdb_gene_annotate(gr) %>% as.tibble()) %>% filter( SYMBOL %in% (filter(., pop %in% paste0("POL_",poly_pop)) %>% pull(SYMBOL))) %>%  View()
...
a_gr <- GRanges(sig_ihs_val %>% filter(genename %in%(ihs_nsl %>% filter(!is.na(n_ihs) & !is.na(n_nsl)) %>% pull(SYMBOL)) ) %>% select(chrom, chrom_start, chrom_end, pop, superpop, genename, statvalue))
subsetByOverlaps(a_gr,a_gr[grep(x = a_gr$superpop, 'POL')]) %>% as.tibble() %>% arrange(seqnames, start) %>% View()

subsetByOverlaps(a_gr,a_gr[grep(x = a_gr$superpop, 'POL')]) %>% as.tibble() %>% arrange(seqnames, start) %>% group_by(seqnames, start) %>% mutate(n = n()) %>% filter(n > 1) %>%ungroup() %>%  mutate(seqnames = as.character(seqnames)) %>% left_join(., markers,by = c('seqnames' = 'chrom', 'start' = 'chrom_start')) %>% View()
...
# nsl
b_gr <- GRanges(sig_nsl_val %>% filter(genename %in%(ihs_nsl %>% filter(!is.na(n_ihs) & !is.na(n_nsl)) %>% pull(SYMBOL)) ) %>% select(chrom, chrom_start, chrom_end, pop, superpop, genename, statvalue))
...
In the clustered significant regions of both \gls{ihs} and \gls{nsl} there were XXX regions that overlapped with genes associated with urate and metabolic diseases. \gls{ihs} had XXX regions, while \gls{nsl} had XXX, and in common between the two statistics were XXX regions. These regions covered XXX \gls{snp}s for \gls{ihs} and XXX for \gls{nsl}, with XXX in common between them.

Using the significant \gls{snp}s for these two statistics (as in Chapter 3) there were a total of XXX \gls{snp}s that were significant in a Polynesian population and also in 

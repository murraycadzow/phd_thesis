...
txdb_gene_annotate <- function(gr){
  ...
  all_transcripts <- GenomicFeatures::transcripts(txdb) # uses txname as column in GR
  #filter for unique regions in gr
  if("marker" %in% colnames(gr)){
  gr <- gr %>%  data.frame() %>% dplyr::select(seqnames, start, end, strand, marker) %>% dplyr::distinct() %>% GenomicRanges::GRanges()
  } else{
    gr <- gr %>%  data.frame() %>% dplyr::select(seqnames, start, end, strand) %>% dplyr::distinct() %>% GenomicRanges::GRanges()
  }
  # find the matching rows between the 2 GR objects
  overlap_hits <- data.frame(GenomicRanges::findOverlaps(gr,all_transcripts))
  # pull out the GENEID column for the matching ucsc transcripts
  matching_transcripts <- AnnotationDbi::select(txdb , 
  ...
  gene_list <- AnnotationDbi::select(org.Hs.eg.db::org.Hs.eg.db, keys =  unique(matching_transcripts$GENEID), columns = c('SYMBOL','ENTREZID'), keytype="ENTREZID")
  ...
  filtered_annotated_ucsc_transcripts <- annotated_ucsc_transcripts %>% dplyr::select(SYMBOL, TXSTRAND, queryHits) %>% dplyr::filter(!is.na(SYMBOL)) %>% dplyr::distinct()

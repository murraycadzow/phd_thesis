...
\citep{Hollis-Moffatt2009, Hollis-Moffatt2011, Phipps-Green2010}. The
...
\citep{Hermisson2005, Hermisson2017, Nielsen2005, Smith1974}.
...
\caption{Representation of alleles before and after a hard
...
\citep{Szpiech2014}. Markers with an \gls{ihs}, \gls{nsl} or \gls{xpehh}
value \textgreater{} \textbar{}2.6\textbar{} met the threshold for
significance, which was equivalent to approximately 1\% of the most
extreme values.
...
\caption{\label{tab:populations}\label{tab:populations} Description of Populations used in the selection analysis (n = 3004).}
\centering
...
\NormalTok{  tmp_list2[[i]] <-}\StringTok{  }\NormalTok{tmp_list2[[i]][ tmp_list2[[i]][,}\DecValTok{2}\NormalTok{] }\OperatorTok{%in%}\StringTok{ }\NormalTok{tmp_list[[i]][,}\DecValTok{2}\NormalTok{],]}
\NormalTok{\}}
...
\citep{voight2006map, sabeti2006positive, Coop2009, tang2007new, pickrell2009signals, Grossman2010}.
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/03-SelectionPipeline_files/figure-latex/crebrf-1.pdf}
\caption{\label{fig:crebrf}Schematic of the \emph{CREBRF} locus with flanking regions,
...
direction of transcription indicated by arrow heads (bottom).}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/03-SelectionPipeline_files/figure-latex/superUpset-1.pdf}
\caption{\label{fig:superUpset}Upset plot showing the intersections of genes that had
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/03-SelectionPipeline_files/figure-latex/polyUpset-1.pdf}
\caption{\label{fig:polyUpset}Upset plot showing the number of genes that have
...
ordered by number of intersecting sets.}
\end{figure}
...
\caption{\label{tab:keggPath}\label{tab:keggPath} Pathway terms that were significant after multiple-testing adjustment in Polynesian populations in Enrichr KEGG 2016 pathway enrichment analysis.}
\centering
\resizebox{\linewidth}{!}{
...
\midrule
ABC transporters &  &  & iHS (9/44) & iHS (9/44), nSL (6/44)\\
Adherens junction &  &  &  & nSL (8/74)\\
Adrenergic signaling in cardiomyocytes & nSL (12/148) & nSL (13/148) &  & \\
Aldosterone synthesis and secretion &  & nSL (8/81) &  & \\
Arrhythmogenic right ventricular cardiomyopathy (ARVC) & nSL (9/74) & nSL (9/74) &  & nSL (10/74)\\
Axon guidance &  &  &  & nSL (13/127)\\
Bacterial invasion of epithelial cells & \gls{td} (11/78) & \gls{td} (11/78) &  & \\
Calcium signaling pathway & nSL (12/180) & nSL (14/180) &  & \\
Cardiac muscle contraction &  & nSL (7/78) &  & \\
cGMP-PKG signaling pathway & nSL (16/167) & nSL (13/167) &  & \\
Cholinergic synapse &  & nSL (10/111) &  & \\
Circadian entrainment &  & nSL (10/95) &  & \\
Dilated cardiomyopathy & nSL (9/90) & nSL (9/90) &  & nSL (9/90)\\
ECM-receptor interaction &  &  &  & iHS (11/82), nSL (8/82)\\
Endocrine and other factor-regulated calcium reabsorption &  & nSL (5/47) &  & \\
Focal adhesion &  &  &  & nSL (17/202)\\
Gap junction &  & nSL (9/88) &  & nSL (9/88)\\
Glutamatergic synapse &  & nSL (13/114) &  & \\
GnRH signaling pathway &  & nSL (7/91) &  & \\
...
with obesity and \gls{t2d} \citep{shungin2015genetic, mahajan2014genome}
and \emph{RREB1} was associated with \gls{t2d} and urate
\citep{mahajan2014genome, Kottgen2013}.
...
\caption{Manhattan plot for significant markers in \gls{cim} for
...
\caption{Manhattan plot for significant markers in \gls{nzm} for
...
\caption{Manhattan plot for significant markers in \gls{sam} for
...
\caption{Manhattan plot for significant markers in \gls{ton} for
...
associated with urate and diseases with \gls{hu} as a co-morbidity, loci
that associated with these conditions were extracted from the \gls{gwas}
catalog (as per section \ref{diseaselist}, see Table
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/03-SelectionPipeline_files/figure-latex/intraSelGenes-1.pdf}
\caption{\label{fig:intraSelGenes}Genes associated with urate, gout, obesity,
...
(haplotypic methods) for the population.}
\end{figure}
...
severe malaria \citep{Timmann2012}, however, this marker had a
...
malaria-associated variant was rs10900585 \citep{Timmann2012}. All three
variants were not in \gls{ld} with each other (\gls{eas}
...
\caption{\label{tab:unnamed-chunk-90}\label{tab:ddcLd} Linkage disequilibrium in EAS populations (R\textsuperscript{2}) between markers in \textit{DDC}.}
\centering
\begin{threeparttable}
...
\caption{A) Principal components 1 and 2 for all populations. B)
...
individual for each population, there were some clear similarities
amongst the super populations (Figure \ref{fig:admix}). Within the
\gls{afr} populations there were three main ancestral populations, which
correlated with geography, with the East African population of \gls{lwk}
being most different to the West African populations (\gls{esn},
\gls{yri}, \gls{gwd}, and \gls{msl}). The populations of \gls{acb} and
\gls{asw} showed signs of admixture with similarities to the ancestral
populations in the West African populations, with smaller proportions
from the ancestral populations of \gls{lwk}, and the European
populations. The \gls{eas} populations were represented by two ancestral
populations. \Gls{cdx} was mostly one, and \gls{jpt} mostly the other,
the remaining \gls{eas} populations were mixtures of the two. A single
ancestral population represented \gls{sas}, but there were indications
of admixture with the ancestral populations of the \gls{eur} and
\gls{eas} populations. In the \gls{eur} populations there was again two
main ancestral populations, they correlated with North and South Europe.
One ancestral population was mostly in \gls{fin}, and the other mostly
in \gls{tsi} and \gls{ibs}. \gls{gbr}, \gls{ceu}, and \gls{nzc} were all
extremely similar in proportions of the two and ancestral populations.
The \gls{amr} populations were mostly differing proportions of the
ancestral population that \gls{tsi} had, and an ancestral population
that was only in the \gls{amr} populations. There were also small
proportions of the ancestral populations that made up the West African
populations. The \gls{pol} populations had two ancestral populations,
one found in the East Polynesian populations, and the other in the West
Polynesian populations. All Polynesian populations had indications of
small amounts of admixture with the ancestral populations of the
\gls{eur} populations, with the East Polynesian populations having a
higher amount than the West Polynesian populations. This analysis was
consistent with the results of the \gls{pca}, and other admixture
analysis \citep{1KGP2015snp, Hellenthal2014}, but also showed that there
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/admix-1.pdf}
\caption{\label{fig:admix}Proportions of ancestral populations as inferred from
...
Clustering on the full chromosome results for all chromosomes from
PopGenome (Figure \ref{fig:multichrplot}) did not group the populations
of each super population together, with the exception of \gls{fwh},
which grouped all super populations, although it split the \gls{amr}
populations into two subgroups. \gls{td} grouped the \gls{eur} and most
of the \gls{sas} super populations, and had smaller sub-super population
groupings of two to three populations. \Gls{cim}, \gls{nzm}, and
\gls{sam} were grouped with two \gls{amr} populations, (\gls{pel} and
\gls{mxl}), and this group was the least similar to the others. There
was no clear pattern to the clustering order of \gls{flf}, although it
did have a few small groups of the same super population of no more than
three individual populations. The \gls{cim} and \gls{sam} populations
were grouped together for both statistics. \Gls{ze} clustered the
\gls{afr} populations together and this cluster was the least similar to
the other populations. There was also a grouping of the \gls{eas}
populations which had the Western Polynesian populations as part of it.
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/multichrplot-1.pdf}
\caption{\label{fig:multichrplot}Hierarchical clustering of the populations using
...
label for each dendrogram. Coloured by super population.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/wholechrfst-1.pdf}
\caption{\label{fig:wholechrfst}\emph{F\textsubscript{ST}} Hierarchical clustering on
chromosomal F\textsubscript{ST} using Euclidean distance and complete
linkage. Coloured by super population.}
\end{figure}
...
significant windows between populations was used for the clustering. For
the haplotypic methods, the proportion of regions that overlapped
between populations was used to create the distance matrix for the
hierarchical clustering. The groups created by the clustering were then
assessed for the number of populations from a super population that were
assigned the same cluster. Having all populations correctly grouped into
their super population was only completely achieved by clustering using
the lower tail of \gls{fwh}. If an allowance is made for the two
\gls{amr} populations to be grouped with the \gls{eur} populations, as
in the chromosomal F\textsubscript{ST} clustering, then both haplotypic
methods and six of the clustered tails of the frequency spectrum-based
methods clustered populations into their respective super populations.

\begin{table}

\caption{\label{tab:proportion}\label{tab:proportion} Proportion of populations clustered into their corresponding super population by selection and neutrality statistic. Proportions were calculated independently for each super population based on the cluster group with the maximum number of populations from a particular super population.}
\centering
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/images/04_clustering/td_densities.png}
\caption{\label{fig:tdDensity}Plot of the distribution of \gls{td} by population.
...
\begin{figure}
\includegraphics[width=6in]{images/04_clustering/tdTop1000} \caption{Hierarchical clustering of \gls{td} using the upper and
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/images/04_clustering/fwh_densities.png}
\caption{\label{fig:fwhDensity}Plot of the distribution of \gls{fwh} by population.
Triangles indicate the minimum and maximum values. Dots indicate the
1\textsuperscript{st} and 99\textsuperscript{th} percentiles for each
population.}
\end{figure}
...
\begin{figure}
\includegraphics[width=6in]{images/04_clustering/fwhTop1000} \caption{Hierarchical clustering of \gls{fwh} using the upper
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/images/04_clustering/flf_densities.png}
\caption{\label{fig:flfDensity}Plot of the distribution of \gls{flf} by population.
Triangles indicate the minimum and maximum values. Dots indicate the
1\textsuperscript{st} and 99\textsuperscript{th} percentiles for each
population.}
\end{figure}
...
\begin{figure}
\includegraphics[width=6in]{images/04_clustering/flfTop1000} \caption{Hierarchical clustering of \gls{flf} using the upper
and lower 1\% of the distribution. \textbf{A} and \textbf{B} Dendrograms
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/images/04_clustering/ze_densities.png}
\caption{\label{fig:zeDensity}Plot of the distribution of \gls{ze} by population.
Triangles indicate the minimum and maximum values. Dots indicate the
1\textsuperscript{st} and 99\textsuperscript{th} percentiles for each
population.}
\end{figure}
...
\begin{figure}
\includegraphics[width=6in]{images/04_clustering/zeTop1000} \caption{Hierarchical clustering of \gls{ze} using the upper and
lower 1\% of the distribution. \textbf{A} and \textbf{B} Dendrograms
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/ihsclust-1.pdf}
\caption{\label{fig:ihsclust}\gls{ihs} Clustering: A. Dendrogram from hierarchical
...
Axes have been ordered according to the hierarchical clustering.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/nslclust-1.pdf}
\caption{\label{fig:nslclust}\gls{nsl} Clustering: A. Dendrogram from hierarchical
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/goutdendros-1.pdf}
\caption{\label{fig:goutdendros}Dendrograms created from hierarchical clustering
applied to windows from the 62 loci associated with urate and gout.
Coloured by super population.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/obesitydendros-1.pdf}
\caption{\label{fig:obesitydendros}Dendrograms created from hierarchical clustering
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/t2ddendros-1.pdf}
\caption{\label{fig:t2ddendros}Dendrograms created from hierarchical clustering
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/kidneydendros-1.pdf}
\caption{\label{fig:kidneydendros}Dendrograms created from hierarchical clustering
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/04-Clustering_files/figure-latex/metsyndendros-1.pdf}
\caption{\label{fig:metsyndendros}Dendrograms created from hierarchical clustering
...
\caption{\label{tab:unnamed-chunk-56}\label{tab:randgenes} Percentage of draws from randomly selected genes for completely clustered super populations.}
\centering
...
expansion
\citep{Nielsen2017, Matisoo-Smith2015, Skoglund2016, Hudjashov2018}.
...
\citep{Tajima1989, fay2000hitchhiking, Fu1993, Zeng2006}. The upper end
...
reported results \citep{1KGP2015snp, pickrell2009signals}. The grouping
...
\citep{Burton2007, Aric1989, Splansky2007}.
...
prioritise \gls{gwas} results \citep[\citet{Field2016a},
\citet{Casto2011}]{Ayodo2007}.
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/05-Selection_and_association_files/figure-latex/ukbbupsetplot-1.pdf}
\caption{\label{fig:ukbbupsetplot}An ``upset plot'' showing the number of samples
(left) and intersections (upper) for the different classification
criteria.}
\end{figure}

\begin{landscape}\begin{table}

\caption{\label{tab:unnamed-chunk-7}\label{tab:ukbbprev} Number, prevalence (95\% CI) of participants defined as gout cases}
\centering
...
\citet{Kottgen2013} revealed similar odds ratios for all gout
definitions (Figures \ref{fig:kotplot} and \ref{fig:kotplot2}, Table
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/05-Selection_and_association_files/figure-latex/kotplot-1.pdf}
\caption{\label{fig:kotplot}Plot showing ORs (95\% CI) for the two largest effect
\glspl{snp} reported in \citet{Kottgen2013} based on gout definitions,
rs2231142 and rs12498742. Original Kottgen reported ORs are shown as red
circles. The reported Kottgen OR was inverted to maintain consistency of
reported effect allele (A1) for rs12498742.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/05-Selection_and_association_files/figure-latex/kotplot2-1.pdf}
\caption{\label{fig:kotplot2}Plot showing ORs (95\% CI) for the 28 \glspl{snp}
reported in \citet{Kottgen2013} based on gout definitions. Original
Kottgen reported ORs are shown as red circles. The reported Kottgen OR
was inverted to maintain consistency of reported effect allele (A1) for
the following markers: rs2307394, rs2079742, rs7224610, rs7188445,
rs1178977, rs6770152, rs7193778, rs164009, rs653178, rs2078267 and
rs478607.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{_bookdown_files/05-Selection_and_association_files/figure-latex/ukOverlap-1.pdf}
\caption{\label{fig:ukOverlap}Overlap of markers that reached nominal genome-wide
...
\caption{Manhattan plot for association with gout,
...
a total of 2402 individuals, with a mean age of 48.6 (SD 14.9), was
60.5\% male, and had a mean \gls{bmi} of 35.0 (SD 8.35). A break down by
gout affection is shown in Table \ref{tab:polyGwasclinc}. There were
only two markers that met the threshold for genome-wide significance,
rs2725215 (risk allele = T, OR = 1.939, 95\% CI = 1.567-2.40, P = 1.162
x 10\textsuperscript{-9}), located in \emph{PKD2}, and rs2231142 (risk
allele = T, OR = 2.306, 95\% CI =1.891-2.81, P = 1.570 x
...
\caption{Manhattan plot for gout GWAS, adjusted for age and sex,
...
\caption{Locus zoom plot of Polynesian gout \gls{gwas} results
...
\citep{Kottgen2013, Okada2012, nakayama2017gwas}. Combining the
...
\citep{Kimura2008, Friedlaender2008, Skoglund2016, Hudjashov2018}. From
...
data \citep{Pybus2015, Schrider2018, Sheehan2016}
...
\begin{table}[!htb]
\caption{Significant markers for \gls{ihs} and \gls{nsl} in Polynesian populations. Table can be found in the electronic supplement as file Appendices/03-ihsnsl-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes that had a significant XP-EHH value in Polynesian populations. Table can be found in the electronic supplement as file Appendices/03-xpehh-Pol.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\caption{\label{tab:unnamed-chunk-11}\label{tab:gwascatref} GWAS catalog studies used. Disease trait as specified in GWAS catalog}\\
\toprule
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 1st percentile \gls{td} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-td-pol-firstper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 99th percentile \gls{td} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-td-pol-ninetyninthper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 1st percentile \gls{fwh} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-fwh-pol-firstper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 99th percentile \gls{fwh} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-fwh-pol-ninetyninethper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 1st percentile \gls{flf} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-flf-pol-firstper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 99th percentile \gls{flf} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-flf-pol-ninetyninethper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 1st percentile \gls{ze} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-ze-pol-firstper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Genes intersecting the windows of the 99th percentile \gls{ze} in Polynesians. Table can be found in the electronic supplement as file Appendices/04-ze-pol-ninetyninethper-genes.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{Positions of regions created by clustering significant markers for \gls{ihs} or \gls{nsl} by population. Table can be found in the electronic supplement as file Appendices/04-ihs-nsl-significant-marker-clusters.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{flf} for windows at \gls{t2d} associated loci. Table can be found in the electronic supplement as file Appendices/04-t2d-flf-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{ze} for windows at \gls{t2d} associated loci. Table can be found in the electronic supplement as file Appendices/04-t2d-ze-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{td} for windows at kidney disease associated loci. Table can be found in the electronic supplement as file Appendices/04-kd-td-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{fwh} for windows at kidney disease associated loci. Table can be found in the electronic supplement as file Appendices/04-kd-fwh-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{flf} for windows at kidney disease associated loci. Table can be found in the electronic supplement as file Appendices/04-kd-flf-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{ze} for windows at kidney disease associated loci. Table can be found in the electronic supplement as file Appendices/04-kd-ze-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{td} for windows at metabolic syndrome associated loci. Table can be found in the electronic supplement as file Appendices/04-metsyn-td-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{fwh} for windows at metabolic syndrome associated loci. Table can be found in the electronic supplement as file Appendices/04-metsyn-fwh-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{flf} for windows at metabolic syndrome associated loci. Table can be found in the electronic supplement as file Appendices/04-metsyn-flf-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\begin{table}[!htb]
\caption{\gls{ze} for windows at metabolic syndrome associated loci. Table can be found in the electronic supplement as file Appendices/04-metsyn-ze-gene-clus.csv}
\centering
  \begin{tabular}{l l}
  \end{tabular}
...
\NormalTok{gwas_genes_ucscGR <-}\StringTok{ }\NormalTok{gwas_genes_ucscGR[}
  \KeywordTok{which}\NormalTok{(gwas_genes_ucscGR}\OperatorTok{@}\NormalTok{seqnames }\OperatorTok{%in%}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}\StringTok{'chr'}\NormalTok{,}\DecValTok{1}\OperatorTok{:}\DecValTok{22}\NormalTok{))]}

\CommentTok{# create the regions for the obesity associated genes}
\NormalTok{obesity_GR <-}\StringTok{ }\NormalTok{gwas_genes_ucscGR[}
\NormalTok{  gwas_genes_ucscGR}\OperatorTok{$}\NormalTok{SYMBOL }\OperatorTok{%in%}\StringTok{ }
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(DISEASE.TRAIT }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Crohn's disease"}\NormalTok{, }\StringTok{'Celiac disease'}\NormalTok{, }
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(DISEASE.TRAIT }\OperatorTok{%in%}\StringTok{  }\KeywordTok{c}\NormalTok{(}\StringTok{"Parkinson's disease"}\NormalTok{, }\StringTok{"Alzheimer's disease"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{select}\NormalTok{(DISEASE.TRAIT, }\KeywordTok{contains}\NormalTok{(}\StringTok{'gene'}\NormalTok{))}

\CommentTok{#entries that have an association with malaria}
\NormalTok{malaria <-}\StringTok{ }\NormalTok{gwas_cat }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(DISEASE.TRAIT }\OperatorTok{%in%}\StringTok{  }\KeywordTok{c}\NormalTok{( }\StringTok{"Malaria"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
...
\NormalTok{gwas_genes_ucscGR <-}\StringTok{ }\NormalTok{gwas_genes_ucscGR[}\KeywordTok{which}\NormalTok{(gwas_genes_ucscGR}\OperatorTok{@}\NormalTok{seqnames }\OperatorTok{%in%}\StringTok{ }

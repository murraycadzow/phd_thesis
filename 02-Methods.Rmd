...
gwas_cat <- read.delim(
  '~/data/gwas_catalog/gwas_catalog_v1.0.1-associations_e89_r2017-06-19.tsv', 
...
# the entries that match the diseases reported in Zhang et al 2013 Table 1
zhang_immune <- gwas_cat %>% 
  filter(DISEASE.TRAIT %in% 
           c("Crohn's disease", 'Celiac disease', "Ulcerative colitis", 
             ...
             "Vitiligo")) %>% select(DISEASE.TRAIT, contains('gene'))

# create the gene regions that matched the genes involved in the diseases from Zhang et al 2013
zhang_immune_GR <- gwas_genes_ucscGR[
  gwas_genes_ucscGR$SYMBOL %in%
    unique(unlist(strsplit(zhang_immune$REPORTED.GENE.S., ', '))),]

# find entries for GWAS that were associated with malaria
malaria <- gwas_cat %>% filter(DISEASE.TRAIT %in%  c( "Malaria")) %>% 
  select(DISEASE.TRAIT, contains('gene'))
...
Individuals from this study were genotyped on Illumina CoreExome v24 \gls{snp} array.

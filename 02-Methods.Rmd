...
dir=/Volumes/scratch/merrimanlab/murray/working_dir/coreExome_selection/NZ_coreExome_1kgp/analysis/PCA/indiv_pops
#combine the chromosomes
bcftools concat -O v -o - --threads 10 $(ls NZ_1KGP.chr*gz | sort -n -t'r' -k2) | bcftools view -O z -o NZ_1KGP_allchr.vcf.gz -s $(grep -v 'NAD\|WPN\|EPN' ../../../data/nz_1kgp.panel | cut -f1 | tr '\n' ',') --force-samples -

#find the independent markers
plink1.9b4.9 --vcf NZ_1KGP_allchr.vcf.gz --maf 0.1 --indep-pairwise 50 5 0.2 --out NZ_1KGP_allchr

touch cases.txt

plink1.9b4.9 --vcf NZ_1KGP_allchr.vcf.gz --extract NZ_1KGP_allchr.prune.in --recode --out NZ_1KGP_allchr_indep --make-pheno cases.txt '*'
...
dir=/Volumes/scratch/merrimanlab/murray/working_dir/coreExome_selection/NZ_coreExome_1kgp/analysis/admixture$
...
The \gls{gcta} v1.26.0 [@gctaref] was used to calculate the proportion of heritability explained. A genetic relationship matrix was calculated 
...
The bam files had the base re-calibration step applied to account for platform specific base bias. The base quality is weighted highly in variant calling algorithms, but different platforms have particular inherent systematic biases. The base re-calibration step employs a machine learning approach to correct for these systematic errors.
...
The haplotype caller is used to call genetic variants. The haplotype caller is capable of calling \gls{snp}s and \gls{indel}s simultaneously by using de-novo local assembly. In GVCF mode the haplotype caller creates an intermediate genome \gls{vcf} (gVCF) file per sample. Each samples gVCF file is then able to be combined and all samples are then jointly genotyped together.
...
The gVCFs were then combined by batch and chromosome using the \gls{gatk} CombineGVCFs module. Once combined the all batch gVCFs were variant called together using the GenotypeGVCF module to create \gls{vcf} files by chromosome.
...
java -Xmx48g -jar \ 
  ...
  -mode SNP \
  -recalFile variant_recal/man001-015.SNP.recal \
  -tranchesFile variant_recal/man001-015.SNP.tranches \
  -rscriptFile variant_recal/man001-015.SNP.plots.R \
  -log variant_recal/man001-015.SNP.log
```
...
java -Xmx48g -jar \
  ...
  -mode INDEL \
  -recalFile variant_recal/man001-015.INDEL.recal \
  -tranchesFile variant_recal/man001-015.INDEL.tranches \
  -rscriptFile variant_recal/man001-015.INDEL.plots.R \
  -log variant_recal/man001-015.INDEL.log
```
...
do 
  java -Xmx12g  \
    ...
    -R  reference_files/FASTA/hs37d5/hs37d5.fa  \
    -input ./variant_called/man001-015_${i}.raw.snps.indels.vcf  \
    -mode SNP  \
    -recalFile ./variant_recal/man001-015.SNP.recal  \
    -tranchesFile ./variant_recal/man001-015.SNP.tranches  \
    -nt 4  \
    -L ${i} \
    -o ./variant_recal/man001-015_${i}.recal.snp.vcf  \
    --ts_filter_level 99.5  \
    -log ./variant_recal/man001-015_${i}.snp.applyrecal.log
done
...
do
  java -Xmx12g \
    ...
    -T ApplyRecalibration \
    -R  reference_files/FASTA/hs37d5/hs37d5.fa \
    -input ./variant_recal/man001-015_${i}.recal.snp.vcf \
    -mode INDEL \
    -recalFile ./variant_recal/man001-015.INDEL.recal \
    -tranchesFile ./variant_recal/man001-015.INDEL.tranches \
    -nt 4 \
    -o ./variant_recal/man001-015_${i}.recal.snp.indel.vcf \
    --ts_filter_level 99.0 \
    -L ${i} \
    -log ./variant_recal/man001-015_${i}.indel.applyrecal.log
done
```
...
knitr::kable(data.frame(ce_pop_table), caption = '(\\#tab:cePopTable) Clinical information for New Zealand Populations in selection analysis',align = 'lcrrrrrr', col.names = c("pop","n","Age","Sex (% Male)","BMI","Waist","% Diabetes","% Gout"), booktabs = TRUE )
```
...
```{r, fig.cap = '(ref:cePCA)', echo = FALSE}
...
The first step was to identify and remove individuals who had failed the QC, and keep only the individuals who had been selected through \gls{pca} and had a genotyping rate of at least 95%. Markers were filtered to remove \gls{indel}s so that only \gls{snp}s remained.
...
Principle components for the merged 1000 Genomes Project data and the New Zealand samples showed in the first two components that the \gls{afr}, \gls{eur}, \gls{sas} super populations all formed separate groupings. \gls{eas} and \gls{pol} were extrememly similar in PC1 but the \gls{pol} populations were spread with PC2, with the majority of the spread being accounted for by the Eastern Polynesian populations whereas the Western Polynesian populations had a tight grouping at -0.025 on PC2, slightly below the \gls{eas} group. This can be observed when PC6 is used to separate the Polynesian populations (Figure \@ref(fig:polPCA))
...
eigen %>%  filter(super_pop == 'EUR') %>% ggplot(., aes(x = PC1, y = PC2, shape = pop, colour = super_pop)) + geom_point(size = 2) + theme_bw() + scale_color_manual(values = c(EUR = '#00BFC4')) + guides(shape  = guide_legend(title = "Population")) + scale_shape_manual(values = c(CEU = 0, NZC = 1, GBR = 2, IBS = 3, FIN = 4, TSI = 5))
```

(ref:polPCA)  Principle components 2 and 6 for the Polynesian populations

```{r polPCA, fig.cap = '(ref:polPCA)', echo = FALSE}
eigen %>%  filter(super_pop == 'POL') %>% ggplot(., aes(x = PC6, y = PC2, shape = pop, colour = super_pop)) + geom_point(size = 2) + theme_bw() + scale_color_manual(values = c(POL = '#619CFF')) + scale_shape_manual(values = c("CIM" = 0,"NZM" = 1, "TON" = 2, "SAM" = 3)) + guides(size = FALSE)
```

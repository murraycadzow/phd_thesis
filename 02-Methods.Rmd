...
```{r, echo = FALSE, fig.cap='(ref:cvproj)'}
...
The \gls{gcta} v1.26.0 [@Yang2011; @Yang2011a] was used to calculate the proportion of heritability explained. A genetic relationship matrix was calculated 
...
`r  lapply(1:22, function(x){diff(markers %>% filter(chrom ==x) %>% arrange(chrom_start) %>% pull(chrom_start))}) %>% unlist() %>%  mean() %>% round_dp(., 0)` = density of markers used

...
In order to efficiently merge the data sets, the overlap in the markers from the 1kgp dataset and the coreExome needed to be found. This was done by 
Lets pull out the markers and alleles from the coreExome data

```
parallel 'zgrep -v "^#" QC1_7-FullMerge1.0and1.1_ExDup_MattBrown_Czech-plus_correctAff_norm.chr{}.phased.vcf.gz | cut -f1,2,4,5 > coreExome_chr{}_biallelic_markers.txt ' ::: $(seq 1 22)
```

And now lets pull out the markers for the 1KGP
```
...
parllel 'bcftools view -O v -m 2 -M 2 -v snps -o - /Volumes/BiocArchive/archive/merrimanlab/reference_files/VCF/1000Genomes_vcf_files/Phase3_March2017/ALL.chr{}.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz | grep -v "^#" | cut -f 1,2,4,5 > 1kgp_chr{}_biallelic_markers.txt ' ::: $(seq 1 22)
```
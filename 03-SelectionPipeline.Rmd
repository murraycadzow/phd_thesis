...
Projects to capture the genetic diversity of humans have sampled populations worldwide but with very small numbers [@HGDP; @Simmons2016]. The Oceania group is represented usually by individuals from XXX Melanesia and the Papua New Guinea highlands and don't include individuals from Polynesia.
...
There is prior evidence of selection at urate associated loci. In the review from @Ramos2017 on natural selection in rheumatic disease, one of the main loci for gout, _SLC2A9_, in the \gls{yri} population had evidence. The region of _BCAS3_ and _NACA2_ also had evidence, orignally found by @Grossman2013 in the Asian population from Hapmap.  

- @Ramos2017 (review) _BCAS3_ and _NACA2_ which was actually identified in Grossman2013 in ASI pop
- @Zhang2013a _ATXN2_ (rs653178) and RREB1 (rs675209)
- jha2015 rs589691?

papers that have used empirical distribution:

There is precident in using the empirical distribution to determine significance, instead of relying purely on the numerical value of the statistic being used. This approach however does need to acknowledge false positives, and also that true selection signals may be lost in the noise. The empiracal distribution approach has been used for multiple studies [@ref; @ref; @ref].
...
Why do humans have have elevated uric acid?

- uricase, the enzyme for metabolising urate further was lost through multiple mutations, a reduction in promoter activity followed by 2 further loss of function mutations. This was all before the divergence of humans from the great apes. The loss of uricase is also found in the great apes and monkeys. 

- parallel silencing in the lesser apes
...
- Possible reasons for the loss @Alvares-Lario2010
    ...
    - potent anti-oxidant which accounts for > 50% of the bloods anti-oxidant activity
    - crystals enhance local immune response - protective against cancer due to the turnover of cells?
    
?? The fructose response for leads to insulin desensitivity @Johnston

metabolic pathway for the formation of urate

Hyperuricemia is has two main components over production of urate, or under excretion. Over production is influcenced by diet, such as high purine or fructose content, or under genetic control. 

Under excretion is the main cause of hyperuricemia, caused by either increased reabsorption or decreased secretion. The ATP powered transporter ABCG2 is responsible for excretion of urate in the gut. There are Polynesian specific variants that lead to a reduction of function in ABCG2.

The kidneys are the organ where the majority of secretion and excretion of urate is performed. In the proximal tuble, on the apical membrane, the urate transporters of XXX reabsorb urate, whereas XXX are responsible for secretion. On the basolateral membrane, XXX, increase intracellular uric acid, but SLC2A9 moves uric acid from intracellular to extracellular. 

Gout is an immune response to the uric acid crystals that are formed from the precipitation of urate from the blood. Hyperuricemia is a prerequisite for the formation of these crystals.

Hyperuricemia in the pacific is seen in eastern and western polynesian, but the prevalence in the ancestral populations of near and remote oceania is reduced. Taiwanese aborignals also have the higher prevalence of Hyperuricemia

The prevalence of HU in Polynesia is also higher, with it are the higher prevalences of the comorbidities of gout, diabetes, obesity, dyslipidemia, kidney disease, and cardio vascular disease. In New Zealand,40% of individuals with gout also had diabetes or CVD.    
...
(ref:polyUpset) Upset plot showing the number of genes that have evidence from \gls{td}, \gls{fwh}, \gls{fld}}, \gls{flf}, \gls{ze}, \gls{ihs}, and \gls{nsl} in the four Polynesian populations.
...
(ref:intraSelGenes) Genes associated with urate, gout, obesity, t2d, kidney disease, and metabolic syndrome having both significance in SFS and haplotypic based methods.
...
Pathway gene-set enrichment analysis on gene lists for each statistic that met the significance thresholds was done using Enrichr. There were a total of 10 pathways that after multiple testing correction had significant p-values. Nine pathways were signifcant for the genelist from \gls{nsl} and had a heart focus based on calcium ion movement in the form of genes for calcium transporters and voltage gate calcium channels. There was a single pathway that was significant from the genes from the 1st percentile of \gls{td} which was for bacterial invasion of epithelial cells.
...
In the 0.1st percentile there were 84 genes for \gls{td}, 56 for \gls{fwh}, 99 for \gls{ze}, and 59 in common between \gls{fld} and \gls{flf}. There were three genesm DNAH1, RGS3, and ZNF704 that were in the 0.1st percentile for each of \gls{fld}, \gls{flf}, \gls{td}, and \gls{ze} 
...
THADA Plot
```{r cimTHADA, fig.cap='(ref:cimTHADA)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4}
#plot_gene("CIM","THADA")
...
#### population distribution description
...
_DNAH10_, _DNAH6_, _RGS3_, and _SUPT3H_ were the only genes that intersected a significant window in \gls{td}, \gls{fwh}, \gls{fld}, \gls{flf}, and \gls{ze}. 


```{r}
# look at the lower 0.1% of windows
lower_sig_stats_binned %>% filter(pop == 'NZM',quant == 0.1) %>% left_join(.,lower_sig_stats, by = c('statname','chrom','chrom_start','chrom_end','pop')) %>% select(statname, SYMBOL) %>% group_by(statname, SYMBOL) %>% distinct()%>% tally() %>% arrange(statname, SYMBOL)  %>% spread(statname, n) %>% mutate(rs = select(., 2:NCOL(.)) %>% rowSums(., na.rm=TRUE)) %>% filter(rs > 3)
```

In the 0.1st percentile there were 96 genes that were shared between \gls{fld} and \gls{flf}. \gls{fwh} had 37 genes, \gls{td} had 104, and \gls{ze} had 107. There were 10 genes that were in common between \gls{fld}, \gls{flf}, \gls{td}, and \gls{ze}.
...
```{r nzmDNAH6, fig.cap='(ref:nzmDNAH6)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4}
#plot_gene("NZM","DNAH6")
...
```{r nzmDNAH6consec, fig.cap='(ref:nzmDNAH6consec)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4}
# g = "DNAH6"
...
```{r nzmSUMPT3H, fig.cap='(ref:nzmSUPT3H)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4}
...
DNAH10 Plot
```{r nzmDNAH10, fig.cap='(ref:nzmDNAH10)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4}
#plot_gene("NZM", "DNAH10")
...
There were only two pathways that were significant for the gene-set pathway enrichment analysis using Enrichr. ABC transporters was significant for gene lists from both \gls{fld} and \gls{ihs}. The second pathway that was significant was long-term potentiation and that was from the \gls{nsl} gene list.
...
```{r samSPAG16, fig.cap='(ref:samSPAG16)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4}
...
```{r samSPECC1, fig.cap='(ref:samSPECC1)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4}
...
```{r samSUPT3H, fig.cap='(ref:samSUPT3H)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4, cache = TRUE}
...
CREB1 Plot
```{r samCREB1, fig.cap='(ref:samCREB1)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4, cache = TRUE}
#plot_gene("SAM","CREB1")
...
```{r tonCENPP, fig.cap='(ref:tonCENPP)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4, cache = TRUE}
#plot_gene("TON","CENPP")
...
```{r tonSH3RF1, fig.cap='(ref:tonSH3RF1)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4, cache = TRUE}
...
```{r tonSUPT3H, fig.cap='(ref:tonSUPT3H)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4, cache = TRUE}
...
THADA Plot
```{r tonTHADA, fig.cap='(ref:tonTHADA)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4, cache = TRUE}
#plot_gene("TON","THADA")
...
ATXN2 ze for CIM, NZM, and SAM. SAM also had fld and flf

BAZ1B td for NZM, SAM and TON

BCAS3 had fwh and flf for NZM, CIM, SAM, TON. fld for CIM, NZM and TON. td for CIM, NZM, SAM.

NFAT5 had NZM for td and flf

PRKAG2 had TON for both fld and SAM for just flf and nsl

RREB1 had td for CIM, NZM and TON. flf for NZM and SAM

SLC16A9 had fld for TON

SLC2A9 had fld and flf for TON

ABCG2 had ihs and nsl for TON

IGFR had 5 markers for TON for nsl, SAM had one marker and it that was different to TON
```{r}
sig_ihs_val %>% filter(pop %in% poly_pop, genename %in% urate_goutGR$SYMBOL)
```
```{r}
sig_nsl_val %>% filter(pop %in% poly_pop, genename %in% urate_goutGR$SYMBOL)
```
...
# NZM fst
pol_fst %>% slice(fst_map@from) %>% bind_cols(., data.frame(urate_goutGR[fst_map@to], stringsAsFactors = FALSE) ) %>% select(contains('chrom'), SYMBOL,contains('NZM') )%>% gather("pop","fst", 5:NCOL(.))%>% mutate(pop = popswitch(pop, "NZM")) %>% mutate(pop1 = substring(pop,1,3), pop2 = substring(pop, 4, 7)) %>% left_join(., panel %>% select('pop1' = pop, super_pop) %>% distinct(), by = 'pop1') %>% distinct() %>% ggplot(., aes(x = chrom_start, y =fst, by = pop1, colour = super_pop)) + geom_line() + facet_wrap(~SYMBOL, scales = 'free') + ggtitle('NZM FST')
```

```{r}
# CIM fst
pol_fst %>% slice(fst_map@from) %>% bind_cols(., data.frame(urate_goutGR[fst_map@to], stringsAsFactors = FALSE) ) %>% select(contains('chrom'), SYMBOL,contains('CIM') )%>% gather("pop","fst", 5:NCOL(.))%>% mutate(pop = popswitch(pop, "CIM")) %>% mutate(pop1 = substring(pop,1,3), pop2 = substring(pop, 4, 7)) %>% left_join(., panel %>% select('pop1' = pop, super_pop) %>% distinct(), by = 'pop1') %>% distinct() %>% ggplot(., aes(x = chrom_start, y =fst, by = pop1, colour = super_pop)) + geom_line() + facet_wrap(~SYMBOL, scales = 'free') + ggtitle('CIM FST')
```

```{r}
# SAM fst
pol_fst %>% slice(fst_map@from) %>% bind_cols(., data.frame(urate_goutGR[fst_map@to], stringsAsFactors = FALSE) ) %>% select(contains('chrom'), SYMBOL,contains('SAM') )%>% gather("pop","fst", 5:NCOL(.))%>% mutate(pop = popswitch(pop, "SAM")) %>% mutate(pop1 = substring(pop,1,3), pop2 = substring(pop, 4, 7)) %>% left_join(., panel %>% select('pop1' = pop, super_pop) %>% distinct(), by = 'pop1') %>% distinct() %>% ggplot(., aes(x = chrom_start, y =fst, by = pop1, colour = super_pop)) + geom_line() + facet_wrap(~SYMBOL, scales = 'free') + ggtitle('SAM FST')
```

```{r}
# TON fst
pol_fst %>% slice(fst_map@from) %>% bind_cols(., data.frame(urate_goutGR[fst_map@to], stringsAsFactors = FALSE) ) %>% select(contains('chrom'), SYMBOL,contains('TON') )%>% gather("pop","fst", 5:NCOL(.))%>% mutate(pop = popswitch(pop, "TON")) %>% mutate(pop1 = substring(pop,1,3), pop2 = substring(pop, 4, 7)) %>% left_join(., panel %>% select('pop1' = pop, super_pop) %>% distinct(), by = 'pop1') %>% distinct() %>% ggplot(., aes(x = chrom_start, y =fst, by = pop1, colour = super_pop)) + geom_line() + facet_wrap(~SYMBOL, scales = 'free') + ggtitle('TON FST')
```


ABCG2 - only tongans have haplotype, no sfs results for pol. AFR had fwh only single pop

SLC2A9 - only chb/jpt/ton and all for only flf fld
...
The regions of consecutive depressed score for \gls{fwh} had a region that intersected _ERBB4_. 

\gls{fld} had regions that intersected _MACROD1_, _DNAH10_, CCDC92, NLRC3, CALCRL, NT5DC2, PBRM1, C6orf106 

\gls{flf} had regions that intersected _DNAH10, ZNF664, CCDC92, NLRC3, CALCRL

\gls{td} had regions intersected by DNAH10, ZNF664, CCDC92, KCTD19, CCR3

\gls{ze} had 2 regions that intersected AGBL4
...
There were three genes that were assiciated with malaria and also had significant ihs scores, these were only in 
...
(ref:ldDDCnzm) Linkage disequillibrium of \gls{ihs} significant SNPs (rs2060762, rs11238131, rs3807558, rs2329371, and rs1966839) in \gls{nzm} at the _DDC_ locus with malaria (rs1451375), BMI, (rs10499694), and blood metabolite ratios (rs7809234).
...
- there is evidence of selection in malaria related loci, that are also assciated with T2D and obesity.
...
The evidence for selection in urate does not appearto be specific for urate itself, but for loci that are involved in more general metabolic pathways. 
...
Zheng2013
SH2B3-ATXN2 has snps associated with urate, and other diseases (rs3184504, rs4766578, rs653178, and rs11065987) are in close LD. derived allele haplotype was assocaiated with increased risk of autoimmune, bp, and urate. hapltoype was common in european but not in afr or asi. possible recent sweep due to bacterial infection @Zhernakova2010. The locus also extends to ALDH2 which @Oota2004 suggests is under selection

...
```{r cimHapMan, out.extra='angle=90', fig.cap = '(ref:cimHapMan)', fig.width = 7, message= FALSE}
#ggsave(filename = 'images/03_selectionpipeline/cimHapMan.png',width = 7, plot = bind_rows(sig_ihs_val, sig_nsl_val) %>% filter(pop == 'CIM') %>% mutate(P = 1- pnorm(abs(statvalue)), statid = case_when(statid == 20 ~ 'nSL', statid== 26 ~ 'iHS')) %>% left_join(markers, by = c('chrom', 'chrom_start')) %>% rename(CHR = chrom, BP = chrom_start, SNP = marker)%>% ggman::ggmanhattan(.) + facet_grid(statid~.) )
...
```{r nzmHapMan, out.extra='angle=90', fig.cap = '(ref:nzmHapMan)', fig.width = 7, message=FALSE}
#ggsave(filename = 'images/03_selectionpipeline/nzmHapMan.png',width = 7, plot = bind_rows(sig_ihs_val, sig_nsl_val) %>% filter(pop == 'NZM') %>% mutate(P = 1- pnorm(abs(statvalue)), statid = case_when(statid == 20 ~ 'nSL', statid== 26 ~ 'iHS')) %>% left_join(markers, by = c('chrom', 'chrom_start')) %>% rename(CHR = chrom, BP = chrom_start, SNP = marker)%>% ggman::ggmanhattan(.) + facet_grid(statid~.)
#       )
...
```{r samHapMan, out.extra='angle=90', fig.cap = '(ref:samHapMan)', fig.width = 7, message = FALSE}
#ggsave(filename = 'images/03_selectionpipeline/samHapMan.png',width = 7, plot =

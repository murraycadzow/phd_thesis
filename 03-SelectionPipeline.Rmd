...
Projects to capture the genetic diversity of humans have sampled populations worldwide, but outside the sample populations of the 1000 Genomes Project are with small numbers for some populations [@Cann2002; @Rosenburg2002; @Mallick2016]. Past studies that have included Oceanic populations have had minimal representation of Polynesian populations, @Kimura2008 only had 24 Tongan individuals. The latest project for human diversity, the Simons Human Diversity Project, did include the New Zealand M$\bar{a}$ori but they were only represented by a single individual and did not feature in many of the analyses [@Mallick2016].
...
An increased level of urate in the blood is called \gls{hu}. The prevalence of \gls{hu} in Polynesian populations is higher than many other populations [@Gosling2014], and with it are higher prevalences of the co-morbidities of gout, diabetes, obesity, dyslipidemia, kidney disease, and \gls{cvd} [@Winnard2013]. In New Zealand, gout prevalence is ~3.1%, however, prevalence in M$\bar{a}$ori and Pacific peoples is 8-9% with at 40% of individuals with gout also had diabetes or \gls{cvd} [@Winnard2013]. Gout is an immune response to the uric acid crystals that are formed from the precipitation of urate from the blood and \gls{hu} is a prerequisite for the formation of these crystals [@Merriman2011a].
...
fdr_low_tab %>% 
  ...
  kable(., col.names = c("Population","Percentile", "FDR (95% CI)", "Percentile", "FDR (95% CI)"), caption = "(\\#tab:fdrpol) False discovery rate for windowed SFS based intra-population statistics by population.", booktabs = TRUE, format = 'latex') %>%
...
kable(., caption = "(\\#tab:voightTab) List of genes used as positive controls for detection of significant \\gls{ihs} regions. Dashes indicate absence in \\cite{voight2006map}. Brackets indicate number of \\gls{snp}s but wasn't reported in \\cite{voight2006map}.)", booktabs = TRUE, format = 'latex', escape = TRUE)
```
...
Pathway terms from the pathway enrichment analysis using the Enrichr KEGG 2016 table showed there was minimal overlap in the terms that were significant between the the Polynesian populations (Table \@ref(tab:keggPath)). There were more pathways in common between the Eastern Polynesian populations than the Western Polynesian populations.
...
kegg_sig %>% 
  ...
  kable(., col.names = c("Pathway Term","CIM","NZM","SAM", "TON"), format = 'latex', booktabs = TRUE, caption = '(\\#tab:keggPath) Pathway terms that were significant in Polynesian populations in Enrichr KEGG 2016 pathway enrichment analysis', longtable = FALSE) %>% kable_styling(., latex_options = 'scale_down') %>% add_header_above(., c("", "Population" = 4))  
```
...
The \gls{cim} population was the populations that had the largest number of markers in a gene that was significant for \gls{ihs}, out of all populations. 
...
The four genes, _CTNNA3_, _RAD51AP2_, _SUPT3H_, and _VSNL1_, had at least one significant marker for \gls{ihs} or \gls{nsl}, and also had a window in the 1st percentile from four or more statistics from \gls{td}, \gls{fwh}, \gls{fld}, \gls{flf}, and \gls{ze}.
...
neuroPol %>%  mutate_all(., function(x){replace(x, is.na(x), 0)}) %>%  filter(genename %in% neurological_keep) %>% mutate_all(., function(x){replace(x, x == 0, "")}) %>%
  kable(., caption = '(\\#tab:neurologicalPol1) Neurological Diseases', col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay & Wu's H", "Fu & Li's D"," Fu & Li's F", "Tajima's D"," Zeng's E"), format = 'latex') %>% kableExtra::row_spec(0, angle = 90) 
...
Other complex diseases such as the neurological, psychiatric, and inflammatory and autoimmune also showed multiple genes that showed evidence of selection. One of the 'themes' that appeared was there was genes were often involved in multiple diseases or gene lists, and often involved with the immune system. 
...
No selection signal was observed at _CREBRF_ in any of the Polynesian populations that were looked at. One reason for this is that the CoreExome chip did not have the specific variants reported in @Minster2016. Downstream from _CREBRF_ there was a consistent signal with what had been reported. This was only found in the Western Polynesian populations and the populations of the \gls{eas} super population so suggests that the dataset had contained the variants that it would have been possible to detect. However, without the two markers reported in @Minster2016 it is unable to be determined if _CREBRF_ had evidence of selection from this data.

The pathway analysis of genes that where in the extremes for the different selection and neutrality statistics showed that pathways involved with cell signalling were the main results for the Polynesian populations, rather than metabolic pathways. The genes that indicated selection, even for obesity and \gls{t2d}, which are traditionally thought of as being the thrifty phenotype, were genes that were largely either neurological or involved in the immune system.
...
As discussed in @Gosling2014, the thrifty gene hypothesis in the context of the Pacific does not fit with the archaeological history, and from this analysis it appears to have no to minimal support from the loci that exhibit signals of possible selection. [@Ayub2014c] has a similar finding with respect to \gls{t2d}, while some individual loci show some signs of selection, it appears as if selection for \gls{t2d} is not the case.
...
With the adaptation of the selection and neutrality tests into genome-wide usage, specifically the sliding window approach, there was the need to balance the number of segregating sites with the window size [@Pybus2014]. This balance is entirely influenced by \gls{snp} density with whole genome (re)sequence data offering 78 million bi-allelic \gls{snp}s in the 1000 Genomes Project Phase 3. However, the CoreExome \gls{snp} chip data had lower density as `r lapply(1:22, function(x){diff(markers %>% filter(chrom ==x) %>% arrange(chrom_start) %>% pull(chrom_start))}) %>% unlist() %>%  mean() %>% round_dp(., 0)` \gls{snp}s/kbp. This means that window size needs to increase for \gls{snp} chip data in order to have the same number of segregating sites. However, as window size increases it also reduces the resolution of the genome and with it the ability to associate a window with particular genomic features.

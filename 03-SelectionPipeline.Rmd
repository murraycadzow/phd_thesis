...
A genelist derived from @voight2006map Table 1 was created to include genes that showed evidence of selection across different combinations of populations \gls{ceu}, \gls{chb},\gls{chs}, \gls{jpt}, and \gls{yri}. All genes except _SPAG4_ were positive for significant \gls{ihs} markers in the appropriate populations they were orignally reported in (Table \@ref(tab:voightTab)) 

(tab:voightTab) List of genes used as positive controls for detection of significant \gls{ihs} regions. Dashes indicate absence in @voight2006map. Brackets indicate number of \gls{snp}s but not wasn't in @voight2006map.
...
sfs_windows_table %>% 
  ...
  knitr::kable(., format = 'latex', col.names = c("Pop",rep(c("Windows","Regions (n)","Genes (n)","Pop. Genes (n)"),2)), booktabs = TRUE, caption = "(\\#tab:sfsWindowsTable) Number of significant regions and genes in Polynesian populations for the intra population SFS statistics.") %>% 
...
```{r}
superDiseaseGenesTab2 <- bind_rows(
   superSFSdisease%>% 
    group_by(pop, super_pop) %>% 
    summarise(urate = sum(urate), obesity = sum(obesity), t2d = sum(t2d), kd = sum(kd), metsyn = sum(metsyn)) %>% 
    mutate(type = "SFS") %>% arrange(super_pop),
...
```{r superDiseaseGenesTab, warning=FALSE}
superDiseaseGenesTab %>% 
  kable(., col.names = c('Population',"Super Pop",'Urate / Gout','Obesity','T2D','Kidney Disease','Metabolic Syndrome','Type'), caption = '(\\#tab:superDiseaseGenesTab) Number of Genes with evidence of possible positive selection in Super populations from intra-population tests from urate and co-morbidities \\gls{gwas} associated loci.', format ='latex', booktab = TRUE, longtable = TRUE) %>% kable_styling(latex_options = 'repeat_header') %>% add_footnote(c("Combined is the number of genes that had both SFS and haplotypic evidence."))
```
...
obesityPol %>% 
  ...
  kable(., caption = '(\\#tab:obesityPol) Obesity associated loci with multiple signs of possible selection in Polynesian populations.', longtable = TRUE, format = 'latex', col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay & Wu's H", "Fu & Li's D"," Fu & Li's F", "Tajima's D"," Zeng's E"), booktabs = TRUE) %>% kableExtra::row_spec(0, angle = 90) %>% add_header_above(., c("","","XP-EHH" = 6, rep("",7))) %>% kable_styling(latex_options = "repeat_header") %>% footnote(., c("XP-EHH is the number of populations from the super population that had at least one marker sigificant in the gene. \\glsdesc{ihs} and nSL are the number of significant markers. \\gls{fwh}, \\gls{fld}, \\gls{flf}, \\gls{td}, and \\gls{ze} are the number of windows intersecting the gene that met the lower threshold."), general_title = "", escape = TRUE)# %>% filter((AFR > 0 | AMR >0 | EAS > 0 | ihs > 0 | nsl > 0) & (!is.na(Fay.Wu.H) | !is.na(Fu.Li.D)| !is.na(Fu.Li.F) | !is.na(Tajima.D) | !is.na(Zeng.E)))
```
...
```{r, t2dPolTab}
t2dPol %>%  mutate_all(., function(x){replace(x, x == 0, "")}) %>% kable(., caption = '(\\#tab:t2dPol) T2D associated loci with multiple signs of possible selection in Polynesian populations.', longtable = TRUE, format = 'latex', booktabs = TRUE, col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay & Wu's H", "Fu & Li's D"," Fu & Li's F", "Tajima's D"," Zeng's E")) %>% kableExtra::row_spec(0, angle = 90) %>%  add_header_above(., c("","","XP-EHH" = 6, rep("",7)))%>% kable_styling(latex_options = "repeat_header") %>% add_footnote(., notation = "number", label = c("XP-EHH is the number of populations from the super population that had at least one marker sigificant in the gene. \\glsdesc{ihs} and nSL are the number of significant markers. \\gls{fwh}, \\gls{fld}, \\gls{flf}, \\gls{td}, and \\gls{ze} are the number of windows intersecting the gene that met the lower threshold.")) # %>% filter((AFR > 0 | AMR >0 | EAS > 0 | ihs > 0 | nsl > 0) & (!is.na(Fay.Wu.H) | !is.na(Fu.Li.D)| !is.na(Fu.Li.F) | !is.na(Tajima.D) | !is.na(Zeng.E)))
```
...
Both \gls{sam} and \gls{nzm} had \gls{snp}s that were significant for \gls{ihs} in _ATP2B4_, \gls{sam} had rs10494845, and \gls{nzm} had rs142206068.
The malaria associated variant was rs10900585 [@Timmann2012]. All three variants are in \gls{ld} (\gls{eas} R^2^ 1.0).

_DDC_ encodes Dopa Decarboxylase and has been associated with malaria [@Jaloow2009] and also \gls{bmi} [@Locke2015]. \gls{nzm} had significant markers for \gls{xpehh} for nearly all other populations, \gls{cim} had fewer populations, but across all super population other than \gls{sas}. The \gls{nzm} had multiple markers within the malaria associated gene, _DDC_, for \gls{nsl}. The markers were rs2060762, rs3807558, rs2329371, and rs1966839. The same markers in addition with rs11238131 and rs4580999 were significant for \gls{ihs}. The marker rs1966839 (reference allele = T, alternative/ancestral allele = C) was in high LD (0.998) with the malaria associated rs1451375 (reference/ancestral allele = C, alternative = A) where the reference and alternative alleles correspond between the \gls{snp}s and both are intronic variants. Under a dominant model the A allele of rs1451375 is protective of severe malaria (OR = 0.75, CI = 0.66-0.85, P = 6 x 10-6) [@Jallow2009]. 


 \gls{nzm} had significant markers for \gls{xpehh} for nearly all other populations, \gls{cim} had fewer populations, but across all super population other than \gls{sas}.
...
```{r, fig.caption= '(ref:ldDDCnzm)', eval = TRUE}
knitr::include_graphics('images/03_selectionpipeline/LDplot-EAS_DDC_nzm_ihs_extras.png')
```
...
- evidence of selction in Polynesian populations
...
There was no evidence of selection in either of the previously identified loci of PPARGC1A and CREBRF in Polynesian populations. The CoreExome chip did not have rs8192678, but it did have rs1873532 which was in high \gls{ld} (\gls{eas} R^2^ = 0.976). 

As discussed in @Gosling2014, the thrifty gene hypothesis in the context of the Pacific does not  
...
No selection signal was observed at _CREBRF_ in any of the Polynesian populations that were looked at. One reason for this is that the CoreExome chip did _CREBRF_ did not have the specific variant the @Minster2016 and the \gls{ihs} signal that had been observed was only for this \gls{snp}. Downstream however there was a consistent signal with what had been reported and was only found in the Western Polynesian populations and the populations of the \gls{eas} super population. Without the two markers reported in _CREBRF_ it is unable to be determined if _CREBRF_ had evidence of selection from this data.
...
In @Myles2011 the PPARGC1A locus was suggested to be a 'thrifty gene', due to the 
...
XXX The intermediate/high frequency variants are over represented comparatively - this will also potentially have an impact on the haplotypic stats - ihs has most power at intermediate, xpehh is near fixation -> depending on the coding of the snp during the qc may cause it to be kicked out

With the adaptation of the selection and neutrality tests into genome-wide usage, specifically the sliding window approach, there was the need to balance the number of segregating sites with the window size [@Pybus2014]. This balance is entirely influenced by \gls{snp} density with whole genome (re)sequence data offering high density 78 million markers, but \gls{snp} chip data being less dense XXX snps/kb. This means that window size needs to increase for \gls{snp} chip data in order to have the same number of segregating sites. However, as window size increases it also reduces the resolution of the genome and with it the ability to associate a window with particular genomic features.


The phasing of the haplotypes for the Polynesian populations made use of the 1000 Genomes Project reference haplotype panel. This panel does not contain Polynesian specific haplotypes and so the phasing is going to be influenced by this.


The method itself of using the empirical distribution and selecting the extremes for a statistic to determine the region under selection does have merit, however, this does assume that selection had occurred. We also don't know the underlying null distribution of the genome. It does need to be acknowledged that using the extremes of the distribution is likely to lead to many false positives. This was somewhat mitigated in this study by using the permutations of the actual data to calculate the expected \gls{fdr}.
...
The marker density may have been able to be improved up though the imputation of the missing markers using a haplotype reference panel. Some of the draw backs of this approach include the 

- larger genes had a higher likelihood of being 'interesting' due to having the chance of a window intersecting it

There was also a higher likelihood of a longer gene being reported as significant due to more windows overlapping the genic region. Not only that, but the window size meant that \gls{snp}s not in the window might not even be near the actual window i.e. -45 and +45 kb away from the edge of the defined window in the worst case. This would lead to some difficulty in definitive location. The flip side to this was that it allows for upstream and downstream effects of a \gls{snp} to be incorporated. There was a trade off between snp density and window size.

Allele frequency methods, and also the haplotypic methods may get confounded by the gout prevalences of the Polynesian sample populations, especially the \gls{cim} and \gls{ton} where the prevalence is up to five fold higher than the general population prevalence for those populations.


- only looking at positive selection


- The use of the gwas catalog - the gwases are largely conducted in European populations so might miss some population specific variants/genes (e.g. Diabetes and Mexicans?)

One of the limitations of using the gwas catalog is that there is a heavy over representation of genes that are associated to disease in Europeans due to the overall bias to \gls{gwas} research being conducted in European populations [@Haga2010; @Popejoy2016]. This can mean that population specific associations are missed [@Weissglas-Volkov2013; @Minster2016]. Genes that are associated with a condition can also be missed if they do not reach genome-wide significance in a \gls{gwas} but in a candidate gene approach are significant. For example, XXX has been shown to be associated with malaria, however at a genome wide level it is not significant.
...
Response to infectious challenge is a key mechanism that has been thought of as a selective pressure. Malarial infection provided a possible challenge to which urate may have been subjected to selective pressure as such provided a viable candidate due to the involvement of urate with malarial resistance. Six loci that were associated with malaria and identified from the \gls{gwas} catalog were investigated for having signals of selection. Three of these genes did have evidence of selection in the Polynesian populations. Two loci that had evidence had also been identified as being associated with obesity and \gls{t2d}. _ABO_ and _ATP24_ did not have strong evidence, and it came from only \gls{ihs}. _DDC_ on the other hand had comparatively strong haplotypic evidence in the Eastern Polynesian populations. The \gls{nzm} had significant \gls{xpehh} results with every other non-Polynesian population and had six significant markers for \gls{ihs}, three of which were in the top 400 most extreme scores. 

rs1966839 was in near perfect \gls{ld} with the malaria associated intronic variant rs1451375 [@Jallow2009]

Big hit in DDC with NZM, ABO and ATP2B4,
those are gwas associated genes

from the obesity list, there was ADCY9, which had a very strong signal for \gls{ihs} for 7 markers in \gls{cim} and 2 markers in \gls{nzm}, were the only populations with significant \gls{ihs} values. 
A variant, rs10775349, in ADCY9 has been associated with malaria [@Maiga2013]

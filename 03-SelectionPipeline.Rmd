...
> add the significant markers to the crebrf diagram

(ref:crebrf) Schematic of the _CREBRF_ locus with flanking regions at chr5:172,600,000-173,000,000. Positions of the markers included on the CoreExome \gls{snp} array are indicated by vertical lines (top). Exons for genes are indicated by rectangles, and direction of transcription indicated by arrow heads (bottom). 

```{r crebrf, fig.cap= '(ref:crebrf)', eval  = TRUE, message =FALSE, warning = FALSE}
...
tracks(Markers = marker_plot, Sig = p2, Genes = p1, heights = c(1,1,5), xlab = "Chr 5 position (bp)") + theme_bw()
...
kegg_sig %>% 
  ...
  kable(., col.names = c("Pathway Term","CIM","NZM","SAM", "TON"), format = 'latex', booktabs = TRUE, caption = '(\\#tab:keggPath) Pathway terms that were significant in Polynesian populations in Enrichr KEGG 2016 pathway enrichment analysis', longtable = FALSE, linesep = "", escape = FALSE) %>% add_header_above(., c("", "Population" = 4)) %>% footnote(general = "Statistic the pathway enrichment was significant for is provided. Numbers in parentheses are number of genes present and the total number of genes in the pathway.", general_title = "") %>% kable_styling(., latex_options = 'scale_down', font_size = 10)   
```
...
##### Pathway enrichment analysis of selected loci in CIM {#cimPath}
...
##### Pathway enrichment analysis of selected loci in NZM {#nzmPath}
...
##### Pathway enrichment analysis of selected loci in SAM {#samPath}
...
##### Pathway enrichment analysis of selected loci in TON {#tonPath}
...
The `r gc_urate_gout_GR$SYMBOL %>% unique() %>% length()` loci identified in the \gls{gwas} catalog (see Table \@ref(tab:gwascatref) for references) as being associated with urate and gout only had a small proportion with any evidence of possible selection. In the Polynesian populations, from the intra-population neutrality and selection statistics, there were `r uratePol %>% pull(genename) %>% unique() %>% length()` genes that had some evidence. However, there were only nine genes with haplotypic evidence, and only one gene (_RREB1_) that had both haplotypic and \gls{sfs}-based evidence. Overall, the Polynesian populations did not have more urate genes with evidence than the other populations (Table \@ref(tab:superDiseaseGenesTab)).
...
uratePol %>% 
  ...
  kable(., caption = '(\\#tab:uratePol) Number of populations, windows, and markers with significant selection and neutrality statistics in Polynesian populations for all GWAS associated genes for urate and gout', col.names = c('Gene','Population','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay and Wu's H", " Fu and Li's F", "Tajima's D"," Zeng's E"), 
        booktabs = TRUE, format ='latex', longtable = TRUE, escape = FALSE, linesep = '') %>% 
  kableExtra::row_spec(0, angle = 90) %>% 
  add_header_above(., c("","","XP-EHH" = 6, rep("",6))) %>% kable_styling(latex_options = 'repeat_header') # %>% filter((AFR > 0 | AMR >0 | EAS > 0 | ihs > 0 | nsl > 0) & (!is.na(Fay.Wu.H) | !is.na(Fu.Li.D)| !is.na(Fu.Li.F) | !is.na(Tajima.D) | !is.na(Zeng.E)))
rm(g)
```
...
obesityPol %>% 
  ...
  mutate(genename = map_chr(genename, ~ cell_spec(.x, italic = TRUE, format = 'latex'))) %>% 
  kable(., caption = '(\\#tab:obesityPol) Obesity-associated loci with multiple signs of possible selection in Polynesian populations.', 
        longtable = TRUE, format = 'latex', , booktabs = TRUE, escape = FALSE, linesep = '',
        col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay and Wu's H"," Fu and Li's F", "Tajima's D"," Zeng's E")) %>% 
  kableExtra::row_spec(0, angle = 90) %>% 
...
t2dPol %>% mutate_all(., function(x){replace(x, x == 0, "")}) %>% 
  mutate(genename = map_chr(genename, ~ cell_spec(.x, italic = TRUE, format = 'latex'))) %>% 
  kable(., caption = '(\\#tab:t2dPol) T2D-associated loci with multiple signs of possible selection in Polynesian populations.', 
        longtable = TRUE, format = 'latex', booktabs = TRUE, escape = FALSE, linesep = '',
        col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay and Wu's H"," Fu and Li's F", "Tajima's D"," Zeng's E")) %>% 
 kableExtra::row_spec(0, angle = 90) %>% 
 add_header_above(., c("","","XP-EHH" = 6, rep("",6)))%>% 
...
kdPol %>%  mutate_all(., function(x){replace(x, x == 0, "")}) %>%
  mutate(genename = map_chr(genename, ~ cell_spec(.x, italic = TRUE, format = 'latex'))) %>% 
  kable(., caption = '(\\#tab:kdPol) kidney', longtable = TRUE, format = 'latex', escape = FALSE,
        col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay & Wu's H", "Fu & Li's F", "Tajima's D"," Zeng's E")) %>% kableExtra::row_spec(0, angle = 90) %>% kable_styling(latex_options = "repeat_header")# %>% filter((AFR > 0 | AMR >0 | EAS > 0 | ihs > 0 | nsl > 0) & (!is.na(Fay.Wu.H) | !is.na(Fu.Li.D)| !is.na(Fu.Li.F) | !is.na(Tajima.D) | !is.na(Zeng.E)))
```
...
metsynPol %>% mutate_all(., function(x){replace(x, x == 0, "")}) %>% 
  mutate(genename = map_chr(genename, ~ cell_spec(.x, italic = TRUE, format = 'latex'))) %>% 
  kable(., caption = '(\\#tab:metsynTab) Metabolic syndrome associated genes with signal of selection', 
        longtable = TRUE, format = 'latex', booktabs = TRUE, escape = FALSE,
        col.names = c('Population','Gene','AFR','AMR','EAS','iHS','nSL',"Fay & Wu's H", " Fu & Li's F", "Tajima's D"," Zeng's E")) %>% kableExtra::row_spec(0, angle = 90) %>% kable_styling(latex_options = "repeat_header")# %>% filter((AFR > 0 | AMR >0 | EAS > 0 | ihs > 0 | nsl > 0) & (!is.na(Fay.Wu.H) | !is.na(Fu.Li.D)| !is.na(Fu.Li.F) | !is.na(Tajima.D) | !is.na(Zeng.E)))
```
...
neuroPol %>% mutate_all(., function(x){replace(x, is.na(x), 0)}) %>% filter(genename %in% neurological_keep) %>% mutate_all(., function(x){replace(x, x == 0, "")}) %>%
  mutate(genename = map_chr(genename, ~ cell_spec(.x, italic = TRUE, format = 'latex'))) %>% 
  kable(., caption = '(\\#tab:neurologicalPol1) Neurological disease associated genes with multiple signals of possible selection in Polynesian populations.',
        booktabs = TRUE, escape = FALSE,
        col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay and Wu's H"," Fu and Li's F", "Tajima's D"," Zeng's E"), format = 'latex') %>% kableExtra::row_spec(0, angle = 90) %>% add_header_above(., c("","","XP-EHH" = 6, rep("",6))) %>% kable_styling(latex_options = "repeat_header")
...
```{r}
 immunePol %>% filter(genename %in% immune_keep) %>% 
  ...
  mutate(genename = map_chr(genename, ~ cell_spec(.x, italic = TRUE, format = 'latex'))) %>% 
 kable(., caption = '(\\#tab:zhangImmune) Genes associated with various inflammatory and autoimmune diseases that showed possible selection from multiple statistics in multiple Polynesian populations.', 
       longtable = TRUE, booktabs = TRUE, escape = FALSE,
       col.names = c('Population','Gene','AFR','AMR','EAS','EUR','POL','SAS','iHS','nSL',"Fay and Wu's H", " Fu and Li's F", "Tajima's D"," Zeng's E"), format = 'latex') %>% kableExtra::row_spec(0, angle = 90) %>% add_header_above(., c("","","XP-EHH" = 6, rep("",6))) %>%  kable_styling(latex_options = "repeat_header")# %>% filter((AFR > 0 | AMR >0 | EAS > 0 | ihs > 0 | nsl > 0) & (!is.na(Fay.Wu.H) | !is.na(Fu.Li.D)| !is.na(Fu.Li.F) | !is.na(Tajima.D) | !is.na(Zeng.E)))
```
...
```{r ldDDCnzm, fig.cap= '(ref:ldDDCnzm)', eval = TRUE}
...
One of the primary objectives of this chapter were to investigate regions that were under selection in Polynesian populations. From the intra-population statistics used, there were between 866 and 974 genes identified in the Polynesian populations that had both intra-population haplotypic and \gls{sfs} statistics that were in the 1^st^ percentile. The regions with possible evidence of selection showed that comparative to the other super populations, the Polynesian populations had the fewest genes (111) that only had evidence within the Polynesian super population, whereas the African populations had the largest number of genes (457) that only had evidence in the African super population (Figure \@ref(fig:superUpset)). The Polynesian populations had similar distributions of the \gls{sfs}-based statistics (explored more in Chapter 4).

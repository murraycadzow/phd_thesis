...
> In the Western Polynesian populations the only pathway that was in common between \gls{sam} and \gls{ton} was ABC transporters. 
...
```{r, eval = FALSE}
cim_nzm_shared_kegg <- kegg_sig %>% filter(pop %in% c("cim", 'nzm')) %>% select(Term, pop, stat, Genes) %>% spread(pop, Genes) %>% filter(!(is.na(cim) | is.na(nzm)))
cim_nzm_shared_kegg %>% select(-nzm) %>% mutate(cim = str_split(cim, ";")) %>% select(cim) %>% purrr::map_dfr(., ~ map_dfr(.x, ~ tibble(gene = .x, pop = 'cim'))) %>% group_by(gene) %>% tally() %>% arrange(desc(n))

...
```{r}
ihs_nsl_sum_tab <- sig_ihs %>% group_by(pop) %>% tally() %>% select(pop, n_ihs_snps = n) %>% left_join(., ihs_clus %>% group_by(pop) %>% tally()%>% select(pop, n_ihs_clus = n), by = 'pop') %>% left_join(., sig_nsl %>% group_by(pop) %>% tally() %>% select(pop, n_nsl_snps = n) , by = 'pop') %>% left_join(., nsl_clus %>% group_by(pop) %>% tally()%>% select(pop, n_nsl_clus = n), by = 'pop') %>% left_join(., sig_ihs %>% select(pop, chrom, chrom_start, chrom_end) %>% inner_join(., sig_nsl %>% select(pop, chrom, chrom_start, chrom_end), by = c('pop','chrom','chrom_start','chrom_end') ) %>% group_by(pop) %>% tally(), by = "pop") 

ihs_nsl_sum_tab %>% mutate(pop = substr(pop, 5,7))%>% kable(., booktabs = TRUE, col.names = c("Population", "iHS SNPs (n)", "iHS Clusters (n)", "nSL SNPs (n)", "nSL Clusters (n)", "Both (n)"), caption = '(\\#tab:iHSnSLsum) Number of significant SNPs and clustered regions for iHS and nSL by population.') %>% kableExtra::group_rows("AFR", 1, 7) %>% kableExtra::group_rows("AMR", 8, 11) %>% kableExtra::group_rows("EAS", 12, 16) %>% kableExtra::group_rows("EUR", 17, 22) %>% kableExtra::group_rows("POL", 23, 26) %>% kableExtra::group_rows("SAS", 27,31)
...
```{r}
reduced_clus <- list()
for(p in unique(ihs_clus$pop)){
  reduced_clus[[p]] <- GenomicRanges::intersect(GRanges(ihs_clus %>% filter(pop == p)), GRanges(nsl_clus %>% filter(pop == p))) %>% data.frame(stringsAsFactors = FALSE) %>% mutate(pop = p)
}
```
...
bind_rows(sig_ihs %>% mutate(statname = 'ihs'), sig_nsl %>% mutate(statname = 'nsl')) %>%
  filter(pop %in% c('POL_CIM','POL_NZM', 'POL_SAM', 'POL_TON'), genename %in% c(urate_goutGR$SYMBOL, t2d_GR$SYMBOL, obesity_GR$SYMBOL, metsyn_GR$SYMBOL)) %>% 
  mutate(pop = substring(pop, 5,8)) %>% 
  mutate(pop = paste(statname,pop)) %>% 
  group_by(genename, pop) %>% tally() %>% 
  arrange(desc(n)) %>% 
  anti_join(., bind_rows(sig_ihs %>% # remove genes that are found in non-Pol populations
                           mutate(statname = 'ihs'), sig_nsl %>% 
                           mutate(statname = 'nsl')) %>% 
              filter(!pop %in% c('POL_CIM','POL_NZM', 'POL_SAM', 'POL_TON')) %>% 
              select(pop, genename), by = c('pop', 'genename') ) %>% 
  spread(pop, n, fill = '') %>% 
  kable(., caption = 'Disease genes with significant hit in only Polynesian populations', longtable = TRUE)
```
...
xpehh %>% select(-popid1, -popid2, -posid, -experimentid, -statvalue) %>% 
  ...
  tally() %>% 
  left_join(., bind_rows(sig_ihs, sig_nsl) %>% 
               mutate(pop = substring(pop, 5,8)) %>% 
...
xpehh %>%  
  ...
  inner_join(., bind_rows(sig_ihs, sig_nsl) %>% mutate(pop = substring(pop, 5,8)) %>% select(pop,genename) %>% distinct(), by = c('pop','genename')) %>%  
  spread(pop, n, fill = 0) %>% 
...
xpehh %>%  
  ...
  inner_join(., bind_rows(sig_ihs, sig_nsl) %>% mutate(pop = substring(pop, 5,8)) %>% select(pop,genename) %>% distinct(), by = c('pop','genename')) %>%  
  spread(pop, n, fill = 0) %>% 
  mutate(diff = abs( (CIM+ NZM)/2 - (SAM + TON)/2  ))%>% filter(diff > quantile(diff, 0.25)) %>% 
...
xpehh %>%  
  ...
  inner_join(., bind_rows(sig_ihs, sig_nsl) %>% mutate(pop = substring(pop, 5,8)) %>% select(pop,genename) %>% distinct(), by = c('pop','genename')) %>%  
  spread(pop, n, fill = "") %>% 
  anti_join(., bind_rows(sig_ihs, sig_nsl) %>% select(pop, genename) %>% filter(!pop %in% c("POL_CIM","POL_NZM","POL_SAM","POL_TON")) %>% distinct() %>% select(genename), by = "genename") %>% 
...
UpSetR::upset(fromList(list(`Tajima's D` = lower_sig_stats %>% filter(pop %in% poly_pop, statname == 'Tajima.D', !is.na(SYMBOL)) %>% .[['SYMBOL']] %>% unique(), 
                            ...
                            iHS = sig_ihs %>% filter(superpop == 'POL', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique(), 
                            nSL =sig_nsl %>% filter(superpop == 'POL', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique())), 
              order.by = "freq", nset = 7, number.angles = 90, set_size.angles = 90, hjust = -0.25, vjust = 0.5)
```
...
lower_sig_stats %>% select(SYMBOL, pop) %>% filter(pop %in% poly_pop) %>% 
  mutate(
    ...
    ) %>% select(SYMBOL, pop, urate, obesity, t2d, kd, metsyn) %>% distinct() %>% group_by(pop) %>% summarise(urate = sum(urate), obesity = sum(obesity), t2d = sum(t2d), kd = sum(kd), metsyn = sum(metsyn)) %>% mutate(type = 'SFS'),


bind_rows(sig_ihs, sig_nsl) %>% mutate(pop = substring(pop, 5,8)) %>% select(pop, 'SYMBOL' = genename)%>% filter(pop %in% poly_pop) %>% 
  mutate(
    urate = ifelse(SYMBOL %in% urate_goutGR$SYMBOL, TRUE, FALSE),
    obesity = ifelse(SYMBOL %in% obesity_GR$SYMBOL, TRUE, FALSE),
    t2d = ifelse(SYMBOL %in% t2d_GR$SYMBOL, TRUE, FALSE),
    kd = ifelse(SYMBOL %in% kd_GR$SYMBOL, TRUE, FALSE),
    metsyn = ifelse(SYMBOL %in% metsyn_GR$SYMBOL, TRUE, FALSE)
    ) %>% select(SYMBOL, pop, urate, obesity, t2d, kd, metsyn) %>% distinct() %>% group_by(pop) %>% summarise(urate = sum(urate), obesity = sum(obesity), t2d = sum(t2d), kd = sum(kd), metsyn = sum(metsyn)) %>% mutate(type = 'Haplotype'),

bind_rows(
  bind_rows(sig_ihs, sig_nsl) %>% mutate(pop = substring(pop, 5,8)) %>% select(pop, 'SYMBOL' = genename),
  ...
  mutate(
    ...
    kd = ifelse(SYMBOL %in% kd_GR$SYMBOL, TRUE, FALSE),
    metsyn = ifelse(SYMBOL %in% metsyn_GR$SYMBOL, TRUE, FALSE)
    ) %>% select(SYMBOL, pop, urate, obesity, t2d, kd, metsyn) %>% distinct() %>% group_by(pop) %>% summarise(urate = sum(urate), obesity = sum(obesity), t2d = sum(t2d), kd = sum(kd), metsyn = sum(metsyn)) %>% mutate(type = 'combined')
...
extract_sig <- function(popname){
  bind_rows(sig_ihs, sig_nsl) %>% mutate(pop  = substring(pop, 5,8)) %>% select(pop, 'SYMBOL' = genename) %>% bind_rows(., lower_sig_stats %>% select(pop, SYMBOL)) %>% filter(pop == popname, !is.na(SYMBOL)) %>% .[['SYMBOL']] %>% unique()
}



extract_sig_super <- function(superpopname){
  bind_rows(sig_ihs, sig_nsl) %>% select(superpop, 'SYMBOL' = genename) %>% bind_rows(., lower_sig_stats %>% left_join(., panel %>% select(pop, "superpop" = super_pop) %>% distinct(), by = 'pop')%>%  select(superpop, SYMBOL)) %>% filter(superpop == superpopname, !is.na(SYMBOL)) %>% .[['SYMBOL']] %>% unique()
}
...
UpSetR::upset(fromList(list(CIM = extract_sig("CIM"), 
                            ...
                            SAM = extract_sig("SAM"), 
                            TON = extract_sig("TON"))),
                             order.by = "freq", nset = 4, number.angles = 90, set_size.angles = 90, hjust = -0.25, vjust = 0.5)
```
...
UpSetR::upset(fromList(superpop_genes),
                             order.by = "freq", nset = length(names(superpop_genes)), number.angles = 90, set_size.angles = 90, hjust = -0.25, vjust = 0.5)
```
...
This compared to the significant \gls{nsl} markers where there were `r sig_nsl %>% anti_join(., sig_ihs %>% filter(superpop != 'POL'), by = c('genename')) %>% filter(superpop == "POL") %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter( n == 4) %>% NROW` genes, that had at least one marker, and were found in only all four Polynesian populations. Within the only Eastern Polynesian populations there were `r sig_nsl %>% anti_join(., sig_nsl %>% filter(!(pop %in% c("POL_SAM","POL_TON"))), by = c('genename')) %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter(n > 0) %>% NROW()` genes that had a significant marker. Within the Western Polynesian populations there were `r sig_nsl %>% anti_join(., sig_nsl %>% filter(!(pop %in% c("POL_CIM","POL_NZM"))), by = c('genename')) %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter(n > 0) %>% NROW()` genes only significant in that group. When the intersection of Polynesian specific genes for both \gls{ihs} and \gls{nsl} was compared, the number of genes in common was `r inner_join(sig_ihs %>% anti_join(., sig_ihs %>% filter(superpop != 'POL'), by = c('genename')) %>% filter(superpop == "POL") %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter( n == 4), sig_nsl %>% anti_join(., sig_ihs %>% filter(superpop != 'POL'), by = c('genename')) %>% filter(superpop == "POL") %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter( n == 4), by = 'genename') %>% NROW()` that had at least 1 \gls{snp} in only the four Polynesian populations.
...
```
There were `r sig_ihs %>% anti_join(., sig_ihs %>% filter(superpop != 'POL'), by = c('genename')) %>% filter(superpop == "POL") %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter( n == 4) %>% NROW()` genes that only had a significant \gls{ihs} value in \gls{pol} populations. But for \gls{nsl} there were `r sig_nsl %>% anti_join(., sig_nsl %>% filter(superpop != 'POL'), by = c('genename')) %>% filter(superpop == "POL") %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter( n == 4) %>% NROW()`, of which `r sum( (sig_ihs %>% anti_join(., sig_ihs %>% filter(superpop != 'POL'), by = c('genename')) %>% filter(superpop == "POL") %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter( n == 4) %>% .[['genename']]) %in% (sig_nsl %>% anti_join(., sig_nsl %>% filter(superpop != 'POL'), by = c('genename')) %>% filter(superpop == "POL") %>% select(pop, genename) %>% distinct() %>% group_by(genename) %>% tally() %>% filter( n == 4) %>%  .[['genename']]))` genes were in common between the two statistics. 
...
PPARGC1A had significant \gls{ihs} 
NZM = rs12650562, rs1873532
ACB = rs1873532
SAM = rs1873532, rs4235308
TON = rs1873532, rs4235308

\gls{nsl}

nzm = rs13122003, rs6821591, rs12650562, rs1873532
sam = rs13122003, rs6821591, rs1873532, rs4235308
ton = rs13122003, rs2279525, rs1873532, rs4235308, rs7656250
msl = rs885395
lwk = rs4235308
mxl = rs2970870
pel = rs2970870
...
UpSetR::upset(fromList(list(`Tajima's D` = lower_sig_stats %>% filter(pop == 'CIM', statname == 'Tajima.D', !is.na(SYMBOL)) %>% .[['SYMBOL']] %>% unique(), 
                            ...
                            iHS = sig_ihs %>% filter(pop == 'POL_CIM', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique(), 
                            nSL =sig_nsl %>% filter(pop == 'POL_CIM', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique())), 
              order.by = "freq", nset = 7, number.angles = 90, set_size.angles = 90, hjust = -0.25, vjust = 0.5, nintersects = NA)
```
...
#ihs
sig_ihs %>% filter(pop == 'POL_CIM', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()
#nsl
sig_nsl %>% filter(pop == 'POL_CIM', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()

# all with at least 1 sfs and 1 haplotypic
bind_rows(bind_rows(sig_ihs, sig_nsl) %>% filter(pop == 'POL_CIM')%>% select(pop, "SYMBOL" = genename ) %>% distinct(), lower_sig_stats %>% filter(pop == 'CIM')%>% select(pop, SYMBOL) %>% distinct()) %>% filter(!is.na(SYMBOL))  %>% group_by(SYMBOL) %>% tally() %>% filter(n == 2) %>% .[['SYMBOL']] %>% cat()
```
...
#ihs
sig_ihs %>% filter(pop == 'POL_NZM', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()
#nsl
sig_nsl %>% filter(pop == 'POL_NZM', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()

# all with at least 1 sfs and 1 haplotypic
bind_rows(bind_rows(sig_ihs, sig_nsl) %>% filter(pop == 'POL_NZM')%>% select(pop, "SYMBOL" = genename ) %>% distinct(), lower_sig_stats %>% filter(pop == 'NZM')%>% select(pop, SYMBOL) %>% distinct()) %>% filter(!is.na(SYMBOL))  %>% group_by(SYMBOL) %>% tally() %>% filter(n == 2) %>% .[['SYMBOL']] %>% cat()
```
...
- nsl main processes "cellular process", "metabolic process"
- all main processes "cellular process", "metabolic process"
- all overrepresentation test: cellular component movement FE=2.87, P = 7.63e-3
...
UpSetR::upset(fromList(list(`Tajima's D` = lower_sig_stats %>% filter(pop == 'NZM', statname == 'Tajima.D', !is.na(SYMBOL)) %>% .[['SYMBOL']] %>% unique(), 
                            ...
                            iHS = sig_ihs %>% filter(pop == 'POL_NZM', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique(), 
                            nSL =sig_nsl %>% filter(pop == 'POL_NZM', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique())), 
              order.by = "freq", nsets = 7, number.angles = 90, set_size.angles = 90, hjust = -0.5, vjust = 0.5 )
```
```{r, include = FALSE}
...
#ihs
sig_ihs %>% filter(pop == 'POL_SAM', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()
#nsl
sig_nsl %>% filter(pop == 'POL_SAM', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()

# all with at least 1 sfs and 1 haplotypic
bind_rows(bind_rows(sig_ihs, sig_nsl) %>% filter(pop == 'POL_SAM')%>% select(pop, "SYMBOL" = genename ) %>% distinct(), lower_sig_stats %>% filter(pop == 'SAM')%>% select(pop, SYMBOL) %>% distinct()) %>% filter(!is.na(SYMBOL))  %>% group_by(SYMBOL) %>% tally() %>% filter(n == 2) %>% .[['SYMBOL']] %>% cat()
```
...
- nsl main processes: "cellular process", "metabolic process"
- all main processes: "cellular process", "metabolic process"
- all overrepresentation test: cellular component movement FE=3.35, P 8.34e-4
...
UpSetR::upset(fromList(list(`Tajima's D` = lower_sig_stats %>% filter(pop == 'SAM', statname == 'Tajima.D', !is.na(SYMBOL)) %>% .[['SYMBOL']] %>% unique(), 
                            ...
                            iHS = sig_ihs %>% filter(pop == 'POL_SAM', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique(), 
                            nSL =sig_nsl %>% filter(pop == 'POL_SAM', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique())), 
              order.by = "freq", nsets = 7, number.angles = 90, set_size.angles = 90, hjust = -0.25, vjust = 0.5 )
```
...
#ihs
sig_ihs %>% filter(pop == 'POL_TON', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()
#nsl
sig_nsl %>% filter(pop == 'POL_TON', !is.na(genename)) %>% .[['genename']] %>% unique() %>% cat()

# all with at least 1 sfs and 1 haplotypic
bind_rows(bind_rows(sig_ihs, sig_nsl) %>% filter(pop == 'POL_TON')%>% select(pop, "SYMBOL" = genename ) %>% distinct(), lower_sig_stats %>% filter(pop == 'TON')%>% select(pop, SYMBOL) %>% distinct()) %>% filter(!is.na(SYMBOL))  %>% group_by(SYMBOL) %>% tally() %>% filter(n == 2) %>% .[['SYMBOL']] %>% cat()
```
...
- nsl main processes: "cellular process", "metabolic process"
- all main processes: "cellular process", "metabolic process"
- all overrepresentation test:
    - cell adhesion -> biological adhesion: FE=6.02 P = 4.63e-11
    - cell-cell signalling FE=4.09 P 1.18e-7
        - cell communication FE = 1.88 P 1.33e-4
...
The PantherDB overrepresentation test using the panther GO-slim biological process annotation dataset found three significant gene-ontology terms that were overrepresented. The first was for level 1 cell adhesion, level 2 biological adhesion with a fold-enrichment of 6.02 (Bonferroni adjusted P = 4.63x10-11). The second was level 1 cell-cell signalling with fold-enrichment of 4.09 (Bonferroni adjusted P = 1.18x10-7) and the third was the level 2 of this term, cell communication, with a fold-enrichment of 1.88 (Bonferroni adjusted P = 1.33x10-4).
...
UpSetR::upset(fromList(list(`Tajima's D` = lower_sig_stats %>% filter(pop == 'TON', statname == 'Tajima.D', !is.na(SYMBOL)) %>% .[['SYMBOL']] %>% unique(), 
                            ...
                            iHS = sig_ihs %>% filter(pop == 'POL_TON', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique(), 
                            nSL =sig_nsl %>% filter(pop == 'POL_TON', !is.na(genename) & genename != "") %>% .[['genename']] %>% unique())), 
              order.by = "freq", nsets = 7, number.angles = 90, set_size.angles = 90, hjust = -0.25, vjust = 0.5)
```
...
```{r, fig.cap='(ref:toNRXN3)', eval= TRUE, message = FALSE, fig.height = 4, fig.width = 4, cache = TRUE}
# g = "NRXN3"
# #retrieve information of the gene of interest
# 
# wh <- genesymbol[g]
...
- many other snps, some in highish LD

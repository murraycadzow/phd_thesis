...
## Selection and Neutrality Statistic
...
) %>% kable(., col.names = c('Population','Urate / Gout','Obesity','T2D','Kidney Disease','Metabolic Syndrome','Type'), caption = 'Number of Genes with evidence of possible positive selection in Polynesians')
```
...
```{r CIMupset, fig.caption = '(ref:CIMupset)'}
...
```{r upsetNzm, fig.caption = 'NZM Upset Plot'}
...
_BCAS3_ had regions of consecutively depressed \gls{td} and \gls{flf} scores. It also had windows from \gls{fwh} and \gls{fld} that were significant, there were no \gls{ihs} or \gls{nsl} \gls{snp}s that reached significance, however, there were \gls{snp}s that were significant for \gls{xpehh} for the \gls{nzm} haplotype against \gls{beb}, \gls{ceu}, and \gls{gbr}. _INHBC_ had regions of consecutively depresed \gls{ze}. 
...
```{r upsetSam, fig.caption = 'SAM Upset Plot'}
...
```{r upsetTon, fig.caption = 'TON Upset Plot'}

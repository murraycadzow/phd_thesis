...
ukbio_gout %>% summarise(per_sex = sum(sex)/NROW(.), mean_age = mean(age, na.rm = TRUE), mean_bmi = mean(bmi, na.rm = TRUE), mean_waistheight_ratio = mean(waist_height_ratio, na.rm = TRUE), mean_waisthip_ratio = mean(waist_hip_ratio, na.rm = TRUE), n = n()) %>% t()
...
models <- list(
  ...
  list(adjuster = "age,eth", model = glm( goutaff ~  age + eth, family = 'binomial',data = ukbio_gout)),
  list(adjuster = "sex,eth", model = glm( goutaff ~ sex_factor +eth, family = 'binomial',data = ukbio_gout)),
  list(adjuster = "age,sex,eth", model = glm( goutaff ~ age + sex_factor + eth, family = 'binomial',data = ukbio_gout)),
  list(adjuster = "bmi,eth", model = glm( goutaff ~ bmi + eth, family = 'binomial',data = ukbio_gout)),
  list(adjuster = "age,sex,bmi,eth", model = glm( goutaff ~ age + sex_factor + bmi + eth, family = 'binomial',data = ukbio_gout))
)


saveRDS(models, file = '~/Git_repos/bookdown_thesis/data/Ukbiobank/pheno_models.RDS')

...
```{r}
polyhaplogwas %>% select(-statid, Chromosome = chrom, Position = pos, Marker = SNP, `Gene name` = genename, A1, `OR [95\\% CI], P` = effect, CIM, NZM, SAM, TON) %>% kable(., format = 'latex', booktabs = TRUE, caption = "(\\#tab:polyhaplogwas) SNPs that had |iHS| or |nSL| > 2 in the New Zealand Polynesian populations and a gout association P > 2 x 10\\textsuperscript{-4} in the Polynesian GWAS.", escape = FALSE) %>% group_rows(., 'iHS', 1, 11) %>% group_rows(., "nSL", 12, 25) %>% footnote(general = "P value is age and sex adjusted. A1 is the effect allele.", escape = FALSE) %>% kable_styling(latex_options = 'scale_down')
```
...
The main peak of association in the Manhattan plots (Figures \@ref(fig:manhatSelfUltAgeSexBmi) and \@ref(fig:polyManhat)) for the \glspl{gwas} of both the UK Biobank, and the New Zealand Polynesians was _ABCG2_, this was consisent with previous \glspl{gwas} for gout, compared to \gls{gwas} for serum urate which has the strongest association with _SLC2A9_ [\@ref]. Combining the haplotypic selection results for \gls{ihs} and \gls{nsl} from the \gls{gbr} population with the UK Biobank gout \gls{gwas}, showed additional evidence at _SLC22A12_, _PTPN11_, _SLC17A3, _SLC17A4_ , and _ADAM10_, but from the \gls{gwas} met the suggestive significance threshold of P < 10^-5^. All except ADAM10 had also previously been reported as being significant at a genome-wide threshold for association with serum urate levels [\@ref]. However, the locus of _ADAM10_, had not been previously associated at a genome-wide threshold with either gout or serum urate levels, but had previously been reported as a strong candidate for positive selection [\@ref]. The role of ADAM10 in the innate immune system 

The supplementation of the New Zealand Polynesian gout \gls{gwas} with the haplotypic selection statistics of \gls{ihs} and \gls{nsl} from the Polynesian populations of \gls{cim}, \gls{nzm}, \gls{sam}, and \gls{ton} did not have any corroboration with the equivalent combination of analyses in the European ancestry results. The \gls{ton} population was the main source of the selection statistics that met the threshold, this could be due to the high proportion of gout patients (> 50%) that were included in the sample. The lead \gls{snp} in _ABCG2_, rs2231142, a known 

PPCDC

had consistent evidence at ADAM10 from both selection statistics, but had not previously


ABCG2 is the usual contender, the other variant rs11034401 is an intergenic variant on chr11
...
- poly has the ibsp/pkd2/abcg2 region as being influencial
...
One of the limiting factors for the combination of the UK Biobank \gls{gwas} with the selection statistics is that the selection statistics were calculated for a greatly reduced number of markers compared to the markers available in the \gls{gwas}, as the \gls{gwas} had been imputed. 

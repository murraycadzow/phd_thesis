...
Genome-wide association studies provide a statistical framework for an association analysis between a phenotype of interest, and a genotypes of a genetic marker, usually using linear regression for a continuous trait, or logistic regression for a dichotomous trait [\@ref]. The association test, in a \gls{gwas} context, is repeated for all available markers across the genome. In epidemiological studies the use of an accurate case definition is important. There is variation in the gout definitions used from the multi-purpose cohort studies that are used for genetic analysis, some well known cohorts include the Wellcome Trust case-control consortium cohort, the Atherosclerosis risk in the community study, and the Framingham Heart Study [@Burton2007;@Aric1989; @Splansky2007]. 
...
As has been previously mentioned (sections \@ref{intro}, \@ref(chapter4)), New Zealand Polynesians have an increased prevalence of a gout and metabolic disease [@Winnard2013]. Of the 28 genetic variants associated with serum urate levels that have been previously reported in European populations, 17 also had significant associations with gout [@Kottgen2013]. In the New Zealand Polynesian population, XXX have replicated the association with gout, with some Polynesian specific effects being noted [@Phipps-Green2016]. The inherent higher prevalence of gout in Polynesian populations but lack of adequate sample sizes to perform \gls{gwas} with sufficient power to detect loci, other than the main effect loci of _ABCG2_ and _SLC2A9_, means that extra approaches are needed to find additional evidence of to help reduce the number of false negative results.


### Use of selection statistics to inform GWAS

\gls{gwas} in the last few years have required to have larger and larger cohorts, in a race to discover the source of 'missing heritability' [\@ref]. The requirement for larger cohorts comes from the inverse relationship between effect-size and statistical power. The source of missing heritability is thought to be found in the polygenic effects of many small effect genetic variants [\@ref]. \gls{gwas} also suffer from the need to control the false positive rate and as such have a stringent assocation significance threshold [\@ref]. One of the side-effects of this threshold is that variants that have a true effect but due to cohort size do not reach statistical significance are lost in the 'noise' of the \gls{gwas} signal. One of the possibilities to be able to identify such variants is to use methodologies that capture the non-additive genetic variance and pair this with the \gls{gwas}. The use of selection statistics in combination with \gls{gwas} is one of these methodologies that can be appled to prioritise \gls{gwas} results [\@ref].
...
The \gls{gwas} was adjusted for age, sex, and the first ten \glspl{pc} calculated from 2,858 ancestry informative markers (as identified by Illumina). Markers were filtered to remove if the minor allele frequency was less than 0.01, or had an \gls{hwe} chi-squared exact test P > 1x10^-6^. The \gls{gwas} was conducted by Dr Tanya Major(Merriman Lab) using Plink v1.9b.
...
(ref:ukOverlap) Overlap of markers that reached nominal genome-wide significance between the different gout classifications. The upper histogram indicates the size of the intersection adn the left histogram indicates the total number of nominally significant \glspl{snp} by gout definition.
...
### Comparison of haplotypic selection with gout \gls{gwas} in European populations
...
the snps that had an |\gls{ihs}| > 2 and a \gls{gwas} P < 1x10^-5% were rs12104449 (OR 1.275 [1.160-1.402], P = 4.638 x 10^-07^), rs505802, rs9734313, and rs559946

PTPN11 ADAM10, SLC22A12
```{r}
...
```{r, echo = TRUE}
ukbb_p05_ihs2 %>% filter(abs(statvalue) > 2.0, P< 1e-4) %>% pull(genename) %>% unique()
```

```{r, echo = TRUE}
ukbb_p05_ihs2 %>% filter(abs(statvalue) > 2.0, P< 0.01) %>% ggplot(., aes(x = -log10(P), y = statvalue)) + geom_point() + geom_vline(xintercept = 3.5)
```
...
### Comparison of haplotypic selection with gout \gls{gwas} in Polynesian populations
...
polyGout <- read_delim('~/data/PolyGwas/polynesian_gout_CoreExome24v1.0-1_rsIDconverted.assoc.logistic.tab', col_names = TRUE, delim = '\t')%>%filter(TEST == "ADD", !(A1 %in% c("I","D") )) %>% separate(., col = "SNP", into = c("SNP","version"), sep = "_", fill = 'right')
```
...
- the snps that fell in the < 10^-4^ range,   
...
- CIM rs7856710 (no gene)
- TON: rs17013182 (IBSP), rs2725220, rs2725215, rs2728108 (PKD2), rs1871744, rs2231142 (ABCG2)

```{r, echo = TRUE}
 sig_nsl_val %>% filter(superpop == 'POL') %>% inner_join(., polyGout %>% filter(P < 1e-4, TEST == "ADD"), by = c("chrom" = "CHR", "chrom_start" = "BP")) %>% select(-gene_chrom, -TEST, -ensgeneid, -STAT, -sig, -statid)
```

nsl + gwas

- only results from western polynesian
- 3 snps in SAM,  rs2725220, rs2725215 both PKD2, and rs4969131 from SLC39A11 
- TON: rs17013182 (IBSP), rs2725220, rs2725215, , rs2728108 (PKD2), rs1871744, rs2231142 (ABCG2), and rs6944596 (CPVL/CHN2)

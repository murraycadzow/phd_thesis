...
kable(m %>% select(-Category), escape = FALSE, format = 'latex',caption = "(\\#tab:gwascatref) GWAS catalog studies used. Disease trait as specified in GWAS catalog", booktabs = TRUE, longtable = TRUE) %>% kable_styling(latex_options = c("repeat_header"), full_width = TRUE) %>% 
  ...
  group_rows("Obesity", 192, 224) %>% group_rows("T2D", 225, 263) #%>% group_rows("Metabolic Syndrome", 19,22) %>% group_rows("Obesity", 23, 55) %>% group_rows("T2D", 56, 94) 
...
```{r, eval = FALSE}
read_csv('Appendices/04-fld-pol-firstper-genes.csv', n_max = 0) %>% kable(format = 'latex', caption = '(\\#tab:polFldNegGenes) Genes intersecting the windows of the 1st percentile \\gls{fld} in Polynesians') %>% footnote('Appendices/04-fld-pol-firstper-genes.csv', threeparttable = TRUE)
```

```{r, eval = FALSE}
read_csv('Appendices/04-fld-pol-ninetyninethper-genes.csv', n_max = 0) %>% kable(format = 'latex', caption = '(\\#tab:polFldPosGenes) Genes intersecting the windows of the 99th percentile \\gls{fld} in Polynesians') %>% footnote('Appendices/04-fld-pol-ninetyninethper-genes.csv', threeparttable = TRUE)
```
...
#### Random window and gene lists proportion and exclusivity tables
...
)) %>% mutate(stat = sapply(stat, function(x){short_to_latex(stat_to_short(x))}), Type = sapply(Type, function(x){switch(x, chr = "Chromosome", neg = "Lower Tail", pos ="Upper Tail",x)})) %>% mutate_if(., is.numeric, funs(sprintf('%.2f',.)))%>%  select(stat, Type, everything()) %>% data.frame %>% arrange(stat) %>%  data.frame %>% knitr::kable(., caption = "(\\#tab:proportion) Proportion", booktabs = TRUE, escape = FALSE)
